...
class PaymentService implements PaymentServiceInterface
{
    ...
    public function paginate(){
        $payments=$this->paymentRepository->getAllPaginate();
        foreach ($payments as $payment) {
            // Kiểm tra nếu thời gian hiện tại đã quá 10 phút so với created_at
            if ($payment->status !== 1 && Carbon::parse($payment->created_at)->addMinutes(10)->isPast()) {
                // Thay đổi trạng thái thành 3 (Expired)
                $this->setStatusById($payment->id,3);
            }
        }
        return $payments;
    }
    public function setStatusById($id, $status){
        DB::beginTransaction();
        try {
            $payment = $this->paymentRepository->updateStatus($id,$status);
            DB::commit();
            return true;
        } catch (\Exception $e) {
