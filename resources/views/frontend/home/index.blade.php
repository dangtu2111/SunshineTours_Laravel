<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">

<a id='mkdf-back-to-top' href='#'>
	<span class="mkdf-icon-stack">
		<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
			viewBox="0 0 22.3 22.3" style="enable-background:new 0 0 22.3 22.3;" xml:space="preserve">
			<g>
				<path d="M10.8,2" />
				<line x1="10.8" y1="20.9" x2="10.8" y2="2" />
				<line x1="11.5" y1="1.3" x2="10.8" y2="2" />
				<line x1="10.8" y1="2" x2="0.9" y2="11.9" />
				<path d="M10.8,2" />
				<line x1="20.7" y1="12" x2="10.8" y2="2" />
			</g>
		</svg>
		<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
			viewBox="0 0 22.3 22.3" style="enable-background:new 0 0 22.3 22.3;" xml:space="preserve">
			<g>
				<path d="M10.8,2" />
				<line x1="10.8" y1="20.9" x2="10.8" y2="2" />
				<line x1="11.5" y1="1.3" x2="10.8" y2="2" />
				<line x1="10.8" y1="2" x2="0.9" y2="11.9" />
				<path d="M10.8,2" />
				<line x1="20.7" y1="12" x2="10.8" y2="2" />
			</g>
		</svg>
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 12">
			<path
				d="M1.5 10.1C1.1 9.5.9 8.7.9 7.6V4.4c0-1.1.2-1.9.6-2.5C2 1.3 2.7 1 3.7 1c1 0 1.7.2 2.1.7s.6 1.2.6 2.1v.6H4.6v-.7c0-.4-.1-.8-.2-1s-.3-.3-.6-.3c-.4 0-.6.1-.7.4-.1.2-.2.6-.2 1v4.3c0 .5.1.8.2 1 .1.2.4.4.7.4.3 0 .6-.1.7-.4.1-.3.2-.6.2-1.1V7h-.9V5.9h2.7v5H5.3l-.2-.9c-.3.7-.8 1-1.6 1-.9 0-1.6-.3-2-.9zM9 10.2c-.4-.6-.6-1.4-.6-2.4V4.2c0-1 .2-1.8.7-2.4.4-.5 1.1-.8 2.1-.8s1.8.3 2.2.8c.4.5.7 1.3.7 2.4v3.6c0 1-.2 1.8-.7 2.4-.4.5-1.1.8-2.2.8-1 0-1.7-.3-2.2-.8zm2.9-1c.1-.2.2-.5.2-.9V3.7c0-.4-.1-.7-.2-.9-.1-.2-.3-.3-.7-.3s-.6.1-.7.3c-.1.2-.2.5-.2.9v4.6c0 .4.1.7.2.9.1.2.3.3.7.3.4.1.6-.1.7-.3z" />
		</svg>
	</span>
</a>

<div class="mkdf-content" style="margin-top: -150px">
	<div class="mkdf-content-inner">
		<div class="mkdf-full-width">
			<div class="mkdf-full-width-inner">
				<div class="mkdf-grid-row">
					<div class="mkdf-page-content-holder mkdf-grid-col-12">
						<div class="wpb-content-wrapper">
							<div class="vc_row wpb_row vc_row-fluid">
								<div class="wpb_column vc_column_container vc_col-sm-12">
									<div class="vc_column-inner">
										<div class="wpb_wrapper">
											<!-- START Main home REVOLUTION SLIDER 6.6.20 -->
											<p class="rs-p-wp-fix"></p>
											@php
											$url=asset('frontend/img/background_nhatho.jpg');
											@endphp
											<rs-module-wrap id="rev_slider_12_1_wrapper" data-source="gallery"
												style="
												visibility:hidden;
												background:transparent;
												padding:0;margin:0px auto;
												margin-top:0;
												margin-bottom:0;
												background-image:url('{{$url}}');background-repeat:no-repeat;background-size:cover;background-position:center center;">
												<rs-module id="rev_slider_12_1" style="" data-version="6.6.20">
													<rs-slides style="overflow: hidden; position: absolute;">
														<rs-slide style="position: absolute;" data-key="rs-23"
															data-title="Slide"
															data-thumb="//wanderland.qodeinteractive.com/wp-content/uploads/2019/11/h1-rev-slide1-bckg-50x100.jpg"
															data-anim="ms:1000;r:0;" data-in="o:0;" data-out="a:false;">
															<img loading="lazy" decoding="async"
																src="{{asset('frontend/wp-content/plugins/revslider/public/assets/assets/dummy.png')}}"
																alt="a" title="h1-rev-slide1-bckg" width="1920"
																height="882" class="rev-slidebg tp-rs-img rs-lazyload"
																data-lazyload="{{asset('frontend/img/background_nhatho.jpg')}}"
																data-no-retina>
															<!--
				--><rs-layer id="slider-12-slide-23-layer-1" data-type="text" data-color="#fff"
																data-xy="x:c;y:m;yo:18px,18px,18px,40px;"
																data-text="w:normal;s:18;l:26;fw:600;a:center;"
																data-dim="w:590px,590px,590px,293px;minh:0px,none,none,none;"
																data-rsp_o="off" data-rsp_bd="off" data-frame_0="y:50;"
																data-frame_1="st:450;sp:1000;"
																data-frame_999="o:0;st:w;sR:7200;"
																style="z-index:8;font-family:'Muli';">Welcome to the dynamic city where light, creativity and opportunity converge. Discover the modern beauty and unique culture of Ho Chi Minh!
															</rs-layer><!--

				--><rs-layer id="slider-12-slide-23-layer-4" data-type="text" data-xy="x:c;y:m;yo:104px,104px,104px,152px;"
																data-text="a:center;" data-dim="w:1299px;"
																data-rsp_o="off" data-rsp_bd="off" data-frame_0="y:50;"
																data-frame_1="st:600;sp:1000;"
																data-frame_999="o:0;st:w;sR:7050;"
																style="z-index:9;font-family:'Roboto';"><a
																	itemprop="url"
																	href="everyday-urban-concrete-jungle/index.html"
																	target="_self"
																	class="mkdf-btn mkdf-btn-medium mkdf-btn-solid mkdf-btn-svg-icon">
																	<span class="mkdf-btn-text">Read
																		more</span>
																	<svg version="1.1" id="Layer_1"
																		xmlns="http://www.w3.org/2000/svg"
																		xmlns:xlink="http://www.w3.org/1999/xlink"
																		x="0px" y="0px" viewBox="0 0 17 17"
																		style="enable-background:new 0 0 17 17;"
																		xml:space="preserve">
																		<g>
																			<path d="M15,1.9" />
																			<line x1="1.7" y1="15.3" x2="15" y2="1.9" />
																			<line x1="16" y1="1.9" x2="15" y2="1.9" />
																			<line x1="15" y1="1.9" x2="1" y2="1.9" />
																			<path d="M15,1.9" />
																			<line x1="15" y1="16" x2="15" y2="1.9" />
																		</g>
																	</svg>
																	<svg version="1.1" id="Layer_1"
																		xmlns="http://www.w3.org/2000/svg"
																		xmlns:xlink="http://www.w3.org/1999/xlink"
																		x="0px" y="0px" viewBox="0 0 17 17"
																		style="enable-background:new 0 0 17 17;"
																		xml:space="preserve">
																		<g>
																			<path d="M15,1.9" />
																			<line x1="1.7" y1="15.3" x2="15" y2="1.9" />
																			<line x1="16" y1="1.9" x2="15" y2="1.9" />
																			<line x1="15" y1="1.9" x2="1" y2="1.9" />
																			<path d="M15,1.9" />
																			<line x1="15" y1="16" x2="15" y2="1.9" />
																		</g>
																	</svg>
																</a>
															</rs-layer><!--

				--><rs-layer id="slider-12-slide-23-layer-12" data-type="text"
																data-xy="x:c;xo:-86px,-88px,-88px,254px;y:b,b,m,t;yo:554px,468px,-168px,70px;"
																data-text="w:normal;s:10;l:10;fw:600;"
																data-dim="w:149px,149px,149px,656px;" data-rsp_o="off"
																data-rsp_bd="off" data-frame_0="y:50;"
																data-frame_1="st:150;sp:1000;"
																data-frame_999="o:0;st:w;"
																style="z-index:12;font-family:'Crimson Text';">
																<div class="vc_row wpb_row vc_row-fluid">
																	<div
																		class="wpb_column vc_column_container vc_col-sm-12">
																		<div class="vc_column-inner">
																			<div class="wpb_wrapper">
																				<div class="mkdf-icon-list-holder "
																					style="margin-bottom: 0px">
																					<a href="2019/10/index.html"
																						target="_blank">
																						<div
																							class="mkdf-il-icon-holder">
																							<span aria-hidden="true"
																								class="mkdf-icon-font-elegant icon_calendar "
																								style="color: #ffffff;font-size: 18px;padding-right: 6px"></span>
																						</div>
																						<p class="mkdf-il-text"
																							style="color: #ffffff;font-size: 21px">
																							October 11, 2019
																						</p>
																					</a>
																				</div>
																			</div>
																		</div>
																	</div>
																</div>
															</rs-layer><!--

				--><a id="slider-12-slide-23-layer-13" class="rs-layer" href="everyday-urban-concrete-jungle/index.html"
																target="_self" rel="nofollow" data-type="text"
																data-xy="x:c;y:m;yo:-55px,-55px,-88px,-78px;"
																data-text="w:normal;s:55,55,55,40;l:65,65,65,50;fw:700;a:left,left,center,center;"
																data-dim="w:auto,auto,673px,400px;" data-rsp_o="off"
																data-rsp_bd="off" data-frame_0="y:50;"
																data-frame_1="st:300;sp:1000;"
																data-frame_999="o:0;st:w;"
																style="z-index:10;font-family:'Oswald';text-transform:uppercase;">
																Welcome to SunShine Ho Chi Minh
															</a>


															<!--

				--><rs-layer id="slider-12-slide-23-layer-14" data-type="text"
																data-xy="x:c;xo:104px,98px,103px,214px;y:b,b,m,m;yo:554px,468px,-168px,-160px;"
																data-text="w:normal;s:10;l:10;fw:600;"
																data-dim="w:187px,187px,187px,583px;" data-rsp_o="off"
																data-rsp_bd="off" data-frame_0="y:50;"
																data-frame_1="st:150;sp:1000;"
																data-frame_999="o:0;st:w;"
																style="z-index:11;font-family:'Crimson Text';">
																<div class="mkdf-icon-list-holder ">
																	<a href="author/alisa-michaels/index.html"
																		target="_blank">
																		<div class="mkdf-il-icon-holder">
																			<span aria-hidden="true"
																				class="mkdf-icon-font-elegant icon_pencil "
																				style="color: #ffffff;font-size: 18px;padding-right: 6px"></span>
																		</div>
																		<p class="mkdf-il-text"
																			style="color: #ffffff;font-size: 21px">
																			by Alisa Michaels</p>
																	</a>
																</div>
															</rs-layer><!--
--> </rs-slide>
														<rs-slide style="position: absolute;" data-key="rs-32"
															data-title="Slide"
															data-thumb="{{asset('frontend/img/background_lanmark.jpg')}}"
															data-anim="ms:1000;r:0;" data-in="o:0;" data-out="a:false;">
															<img loading="lazy" decoding="async"
																src="{{asset('frontend/img/background_lanmark.jpg')}}"
																alt="a" title="h1-rev-slide2-bckg" width="1920"
																height="882" class="rev-slidebg tp-rs-img rs-lazyload"
																data-lazyload="{{asset('frontend/img/background_lanmark.jpg')}}"
																data-no-retina>
															<!--
				--><rs-layer id="slider-12-slide-32-layer-1" data-type="text" data-color="#fff"
																data-xy="x:c;y:m;yo:18px,18px,18px,55px;"
																data-text="w:normal;s:18;l:26;fw:600;a:center;"
																data-dim="w:590px,590px,590px,293px;minh:0px,none,none,none;"
																data-rsp_o="off" data-rsp_bd="off" data-frame_0="y:50;"
																data-frame_1="st:450;sp:1000;"
																data-frame_999="o:0;st:w;sR:7200;"
																style="z-index:8;font-family:'Muli';">Explore the rich culinary journey of Vietnam, where each dish tells a story full of colour, flavour and tradition. Experience the amazing diversity from street food to unique specialties!
															</rs-layer><!--

				--><rs-layer id="slider-12-slide-32-layer-4" data-type="text" data-xy="x:c;y:m;yo:104px,104px,104px,168px;"
																data-text="a:center;" data-dim="w:1299px;"
																data-rsp_o="off" data-rsp_bd="off" data-frame_0="y:50;"
																data-frame_1="st:600;sp:1000;"
																data-frame_999="o:0;st:w;sR:7050;"
																style="z-index:9;font-family:'Roboto';"><a
																	itemprop="url"
																	href="amazon-rainforest-experience/index.html"
																	target="_self"
																	class="mkdf-btn mkdf-btn-medium mkdf-btn-solid mkdf-btn-svg-icon">
																	<span class="mkdf-btn-text">Read
																		more</span>
																	<svg version="1.1" id="Layer_1"
																		xmlns="http://www.w3.org/2000/svg"
																		xmlns:xlink="http://www.w3.org/1999/xlink"
																		x="0px" y="0px" viewBox="0 0 17 17"
																		style="enable-background:new 0 0 17 17;"
																		xml:space="preserve">
																		<g>
																			<path d="M15,1.9" />
																			<line x1="1.7" y1="15.3" x2="15" y2="1.9" />
																			<line x1="16" y1="1.9" x2="15" y2="1.9" />
																			<line x1="15" y1="1.9" x2="1" y2="1.9" />
																			<path d="M15,1.9" />
																			<line x1="15" y1="16" x2="15" y2="1.9" />
																		</g>
																	</svg>
																	<svg version="1.1" id="Layer_1"
																		xmlns="http://www.w3.org/2000/svg"
																		xmlns:xlink="http://www.w3.org/1999/xlink"
																		x="0px" y="0px" viewBox="0 0 17 17"
																		style="enable-background:new 0 0 17 17;"
																		xml:space="preserve">
																		<g>
																			<path d="M15,1.9" />
																			<line x1="1.7" y1="15.3" x2="15" y2="1.9" />
																			<line x1="16" y1="1.9" x2="15" y2="1.9" />
																			<line x1="15" y1="1.9" x2="1" y2="1.9" />
																			<path d="M15,1.9" />
																			<line x1="15" y1="16" x2="15" y2="1.9" />
																		</g>
																	</svg>
																</a>
															</rs-layer><!--

				--><rs-layer id="slider-12-slide-32-layer-12" data-type="text"
																data-xy="x:c;xo:-85px,-89px,-95px,3px;y:b,b,m,t;yo:554px,468px,-168px,38px;"
																data-text="w:normal;s:10;l:10;fw:600;"
																data-dim="w:149px;" data-rsp_o="off" data-rsp_bd="off"
																data-frame_0="y:50;" data-frame_1="st:150;sp:1000;"
																data-frame_999="o:0;st:w;"
																style="z-index:12;font-family:'Crimson Text';">
																<div class="mkdf-icon-list-holder ">
																	<a href="2019/10/index.html" target="_blank">
																		<div class="mkdf-il-icon-holder">
																			<span aria-hidden="true"
																				class="mkdf-icon-font-elegant icon_calendar "
																				style="color: #ffffff;font-size: 18px;padding-right: 6px"></span>
																		</div>
																		<p class="mkdf-il-text"
																			style="color: #ffffff;font-size: 21px">
																			October 11, 2019</p>
																	</a>
																</div>
															</rs-layer><!--

				--><a id="slider-12-slide-32-layer-13" class="rs-layer" href="amazon-rainforest-experience/index.html"
																target="_self" rel="nofollow" data-type="text"
																data-xy="x:c;y:m;yo:-55px,-55px,-88px,-88px;"
																data-text="w:normal;s:55,55,55,40;l:65,65,65,50;fw:700;a:left,left,center,center;"
																data-dim="w:auto,auto,673px,343px;" data-rsp_o="off"
																data-rsp_bd="off" data-frame_0="y:50;"
																data-frame_1="st:300;sp:1000;"
																data-frame_999="o:0;st:w;"
																style="z-index:10;font-family:'Oswald';text-transform:uppercase;">Vietnam's culinary adventure.
															</a><!--

				--><rs-layer id="slider-12-slide-32-layer-14" data-type="text"
																data-xy="x:c;xo:125px,123px,122px,43px;y:b,b,m,t;yo:554px,468px,-168px,69px;"
																data-text="w:normal;s:10;l:10;fw:600;"
																data-dim="w:229px;" data-rsp_o="off" data-rsp_bd="off"
																data-frame_0="y:50;" data-frame_1="st:150;sp:1000;"
																data-frame_999="o:0;st:w;"
																style="z-index:11;font-family:'Crimson Text';">
																<div class="mkdf-icon-list-holder ">
																	<a href="author/alisa-michaels/index.html"
																		target="_blank">
																		<div class="mkdf-il-icon-holder">
																			<span aria-hidden="true"
																				class="mkdf-icon-font-elegant icon_pencil "
																				style="color: #ffffff;font-size: 18px;padding-right: 6px"></span>
																		</div>
																		<p class="mkdf-il-text"
																			style="color: #ffffff;font-size: 21px">
																			by Alisa Michaels</p>
																	</a>
																</div>
															</rs-layer><!--
--> </rs-slide>
														<rs-slide style="position: absolute;" data-key="rs-33"
															data-title="Slide"
															data-thumb="{{asset('frontend/img/background.jpg')}}"
															data-anim="ms:1000;r:0;" data-in="o:0;" data-out="a:false;">
															<img loading="lazy" decoding="async"
																src="{{asset('frontend/wp-content/plugins/revslider/public/assets/assets/dummy.png')}}"
																alt="a" title="h1-rev-slide3-bckg" width="1920"
																height="882" class="rev-slidebg tp-rs-img rs-lazyload"
																data-lazyload="{{asset('frontend/img/background.jpg')}}"
																data-no-retina>
															<!--
				--><rs-layer id="slider-12-slide-33-layer-1" data-type="text" data-color="#fff"
																data-xy="x:c;y:m;yo:18px,18px,18px,37px;"
																data-text="w:normal;s:18;l:26;fw:600;a:center;"
																data-dim="w:590px,590px,590px,293px;minh:0px,none,none,none;"
																data-rsp_o="off" data-rsp_bd="off" data-frame_0="y:50;"
																data-frame_1="st:450;sp:1000;"
																data-frame_999="o:0;st:w;sR:7200;"
																style="z-index:8;margin-top:30px;font-family:'Muli';">Every step is an exciting challenge and every moment brings surprises. Be ready to face new opportunities, explore the unseen and push all limits!
															</rs-layer><!--

				--><rs-layer id="slider-12-slide-33-layer-4" data-type="text" data-xy="x:c;y:m;yo:104px,104px,104px,149px;"
																data-text="a:center;" data-dim="w:1299px;"
																data-rsp_o="off" data-rsp_bd="off" data-frame_0="y:50;"
																data-frame_1="st:600;sp:1000;"
																data-frame_999="o:0;st:w;sR:7050;"
																style="z-index:9;font-family:'Roboto';"><a
																	itemprop="url"
																	href="climbing-in-the-swiss-alps/index.html"
																	target="_self"
																	class="mkdf-btn mkdf-btn-medium mkdf-btn-solid mkdf-btn-svg-icon">
																	<span class="mkdf-btn-text">Read
																		more</span>
																	<svg version="1.1" id="Layer_1"
																		xmlns="http://www.w3.org/2000/svg"
																		xmlns:xlink="http://www.w3.org/1999/xlink"
																		x="0px" y="0px" viewBox="0 0 17 17"
																		style="enable-background:new 0 0 17 17;"
																		xml:space="preserve">
																		<g>
																			<path d="M15,1.9" />
																			<line x1="1.7" y1="15.3" x2="15" y2="1.9" />
																			<line x1="16" y1="1.9" x2="15" y2="1.9" />
																			<line x1="15" y1="1.9" x2="1" y2="1.9" />
																			<path d="M15,1.9" />
																			<line x1="15" y1="16" x2="15" y2="1.9" />
																		</g>
																	</svg>
																	<svg version="1.1" id="Layer_1"
																		xmlns="http://www.w3.org/2000/svg"
																		xmlns:xlink="http://www.w3.org/1999/xlink"
																		x="0px" y="0px" viewBox="0 0 17 17"
																		style="enable-background:new 0 0 17 17;"
																		xml:space="preserve">
																		<g>
																			<path d="M15,1.9" />
																			<line x1="1.7" y1="15.3" x2="15" y2="1.9" />
																			<line x1="16" y1="1.9" x2="15" y2="1.9" />
																			<line x1="15" y1="1.9" x2="1" y2="1.9" />
																			<path d="M15,1.9" />
																			<line x1="15" y1="16" x2="15" y2="1.9" />
																		</g>
																	</svg>
																</a>
															</rs-layer><!--

				--><rs-layer id="slider-12-slide-33-layer-12" data-type="text"
																data-xy="x:c;xo:-86px,-87px,-92px,0;y:b,b,m,t;yo:554px,468px,-168px,70px;"
																data-text="w:normal;s:10;l:10;fw:600;"
																data-dim="w:149px;" data-rsp_o="off" data-rsp_bd="off"
																data-frame_0="y:50;" data-frame_1="st:150;sp:1000;"
																data-frame_999="o:0;st:w;"
																style="z-index:12;font-family:'Crimson Text';">
																<div class="mkdf-icon-list-holder ">
																	<a href="2019/10/index.html" target="_blank">
																		<div class="mkdf-il-icon-holder">
																			<span aria-hidden="true"
																				class="mkdf-icon-font-elegant icon_calendar "
																				style="color: #ffffff;font-size: 18px;padding-right: 6px"></span>
																		</div>
																		<p class="mkdf-il-text"
																			style="color: #ffffff;font-size: 21px">
																			October 11, 2019</p>
																	</a>
																</div>
															</rs-layer><!--

				--><a id="slider-12-slide-33-layer-13" class="rs-layer" href="climbing-in-the-swiss-alps/index.html" target="_self"
																rel="nofollow" data-type="text"
																data-xy="x:c;y:m;yo:-55px,-55px,-88px,-81px;"
																data-text="w:normal;s:55,55,55,40;l:65,65,65,50;fw:700;a:left,left,center,center;"
																data-dim="w:auto,auto,584px,326px;" data-rsp_o="off"
																data-rsp_bd="off" data-frame_0="y:50;"
																data-frame_1="st:300;sp:1000;"
																data-frame_999="o:0;st:w;"
																style="z-index:10;font-family:'Oswald';text-transform:uppercase;">Exciting challenges and full of surprises
															</a><!--

				--><rs-layer id="slider-12-slide-33-layer-14" data-type="text"
																data-xy="x:c;xo:120px,119px,119px,40px;y:b,b,m,t;yo:554px,468px,-168px,101px;"
																data-text="w:normal;s:10;l:10;fw:600;"
																data-dim="w:225px;" data-rsp_o="off" data-rsp_bd="off"
																data-frame_0="y:50;" data-frame_1="st:150;sp:1000;"
																data-frame_999="o:0;st:w;"
																style="z-index:11;font-family:'Crimson Text';">
																<div class="mkdf-icon-list-holder ">
																	<a href="author/alisa-michaels/index.html"
																		target="_blank">
																		<div class="mkdf-il-icon-holder">
																			<span aria-hidden="true"
																				class="mkdf-icon-font-elegant icon_pencil "
																				style="color: #ffffff;font-size: 18px;padding-right: 6px"></span>
																		</div>
																		<p class="mkdf-il-text"
																			style="color: #ffffff;font-size: 21px">
																			by Alisa Michaels</p>
																	</a>
																</div>
															</rs-layer><!--
--> </rs-slide>
													</rs-slides>
													<rs-static-layers><!--
		--></rs-static-layers>
												</rs-module>
												
											</rs-module-wrap>
											<!-- END REVOLUTION SLIDER -->
										</div>
									</div>
								</div>
							</div>
							<div class="vc_row wpb_row vc_row-fluid">
								<div class="bg-image-over wpb_column vc_column_container vc_col-sm-12">
									<div class="vc_column-inner vc_custom_1574862394737">
										<div class="wpb_wrapper">
											<div class="mkdf-single-image-holder   ">
												<div class="mkdf-si-inner">
													<img loading="lazy" decoding="async" width="1919" height="145"
														src="{{asset('frontend/wp-content/uploads/2019/11/h1-rev-bottom.png')}}"
														class="attachment-full size-full" alt="a"
														srcset="{{asset('frontend/wp-content/uploads/2019/11/h1-rev-bottom.png')}} 1919w, {{asset('frontend/wp-content/uploads/2019/11/h1-rev-bottom-300x23.png')}} 300w, {{asset('frontend/wp-content/uploads/2019/11/h1-rev-bottom-768x58.png')}} 768w, {{asset('frontend/wp-content/uploads/2019/11/h1-rev-bottom-1024x77.png')}} 1024w,  {{asset('frontend/wp-content/uploads/2019/11/h1-rev-bottom-600x45.png')}} 600w"
														sizes="(max-width: 1919px) 100vw, 1919px" />
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
							<div class="vc_row wpb_row vc_row-fluid vc_custom_1572593905416 vc_row-has-fill">
								<div class="wpb_column vc_column_container vc_col-sm-12 vc_col-lg-offset-0">
									<div class="vc_column-inner">
										<div class="wpb_wrapper">
											<div class="mkdf-row-grid-section-wrapper ">
												<div class="mkdf-row-grid-section">
													<div class="vc_row wpb_row vc_inner vc_row-fluid">
														<div
															class="wpb_column vc_column_container vc_col-sm-10 vc_col-lg-offset-3 vc_col-lg-6 vc_col-md-offset-1 vc_col-md-10 vc_col-sm-offset-1">
															<div class="vc_column-inner">
																<div class="wpb_wrapper">
																	<div class="mkdf-section-title-holder   mkdf-st-highlight mkdf-center-align"
																		style="text-align: center">
																		<div class="mkdf-st-inner">
																			<span class="mkdf-st-tagline">Lorem
																				ipsum dolor</span>
																			<h2 class="mkdf-st-title">
																				Travel essentials <span
																					class="mkdf-st-highlight-title-holder"
																					style="color: #f2f1e7">
																					<span
																						class="mkdf-st-highlight-title"
																						style="color: #59815b">tips</span>
																					<span class="mkdf-st-highlight">
																						<svg version="1.1"
																							xmlns="http://www.w3.org/2000/svg"
																							xmlns:xlink="http://www.w3.org/1999/xlink"
																							x="0px" y="0px"
																							viewBox="0 0 15.7 30"
																							style="enable-background:new 0 0 15.7 30;"
																							xml:space="preserve"
																							class="mkdf-active-hover-left">
																							<polygon class="st0"
																								points="2.6,1 0.7,3.3 2,5.8 2.3,7.6 2.9,8.7 4.4,10.5 3.9,10.8 4.4,11.9 4.4,12.8 4.1,13.8 3.3,14.7 3.9,15.8 4.4,16.8 4,17.5 3.5,18.1 2.2,20.2 3.4,21.5 4.2,24.1 3.4,25.4 2.5,27.4 2.5,27.8 3.2,28.3 4.1,28.5 4.9,29 14.8,29 14.8,1 " />
																						</svg>
																						<span
																							class="mkdf-active-hover-middle"></span>
																						<svg version="1.1"
																							xmlns="http://www.w3.org/2000/svg"
																							xmlns:xlink="http://www.w3.org/1999/xlink"
																							x="0px" y="0px"
																							viewBox="0 0 13.3 30"
																							style="enable-background:new 0 0 13.3 30;"
																							xml:space="preserve"
																							class="mkdf-active-hover-right">
																							<polygon class="st0"
																								points="10,1 10.2,2.1 10.6,2.9 10.6,3.3 10.8,3.7 10.8,4.3 11,5 11,5.7 11,6.3 10.5,6.7 10.8,7.3 11,7.8 	11.6,8.3 11.6,8.6 11.5,8.9 11.6,9.9 11.6,10.5 12.4,11.6 12.1,12 12.4,12.2 11.8,12.8 11.4,13.5 11.6,13.7 11.9,13.7 12,13.9 11.5,15.1 10.8,16 9.1,17.7 9.7,18.2 9.3,19 9.7,19.8 9.6,20.6 9.7,21.5 9.6,21.9 9.6,22.3 10.1,22.8 9.6,23.6 9.7,24 9.7,24.2 9.9,24.4 9.5,24.7 9.3,25.4 9.3,25.9 8.8,26.2 8.5,27.1 8.8,27.8 9.4,28.6 7.8,29 0.9,29 0.9,1 " />
																						</svg>
																					</span>
																				</span> </h2>
																		</div>
																	</div>
																</div>
															</div>
														</div>
													</div>
												</div>
											</div>
											<div class="mkdf-row-grid-section-wrapper ">
												<div class="mkdf-row-grid-section">
													<div
														class="vc_row wpb_row vc_inner vc_row-fluid vc_custom_1570542642144">
														<div class="wpb_column vc_column_container vc_col-sm-12">
															<div class="vc_column-inner">
																<div class="wpb_wrapper">
																	<div
																		class="mkdf-elements-holder   mkdf-one-column  mkdf-responsive-mode-768 ">
																		<div class="mkdf-eh-item    "
																			data-item-class="mkdf-eh-custom-4560"
																			data-769-1024="0 0 50px"
																			data-681-768="0 0 45px" data-680="0 0 40px">
																			<div class="mkdf-eh-item-inner">
																				<div class="mkdf-eh-item-content mkdf-eh-custom-4560"
																					style="padding: 0 0 110px">
																					<div class="mkdf-blog-list-holder mkdf-grid-list mkdf-bl-info-right mkdf-one-columns mkdf-disable-bottom-space mkdf-normal-space mkdf-bl-pag-no-pagination "
																						data-type=info-right
																						data-number-of-posts=1
																						data-number-of-columns=one
																						data-space-between-items=normal
																						data-category=travel-tips
																						data-orderby=date data-order=ASC
																						data-image-size=full
																						data-title-tag=h5
																						data-excerpt-length=47
																						data-post-info-section=yes
																						data-post-info-image=yes
																						data-post-info-author=yes
																						data-post-info-date=yes
																						data-post-info-category=yes
																						data-post-info-comments=no
																						data-post-info-like=no
																						data-post-info-share=no
																						data-pagination-type=no-pagination
																						data-max-num-pages=2
																						data-next-page=2>
																						<div
																							class="mkdf-bl-wrapper mkdf-outer-space">
																							<ul class="mkdf-blog-list">
																								<li
																									class="mkdf-bl-item mkdf-item-space">
																									<div
																										class="mkdf-bli-inner">
																										<div
																											class="mkdf-bli-wrapper">

																											<div
																												class="mkdf-post-image">
																												<a itemprop="url"
																													href="{{route('blog.blog_dentail',['id'=>1])}}"
																													title="Moving trough: the everyday urban jungle of your city">
																													<img loading="lazy"
																														decoding="async"
																														width="1300"
																														height="865"
																														src="{{asset('frontend/img/4x3/ho-chi-minh-city.jpg')}}"
																														class="attachment-full size-full wp-post-image"
																														alt="s"
																														srcset="{{asset('frontend/img/4x3/ho-chi-minh-city.jpg')}} 1300w, {{asset('frontend/img/4x3/ho-chi-minh-city.jpg')}} 600w, {{asset('frontend/img/4x3/ho-chi-minh-city.jpg')}} 300w, {{asset('frontend/img/4x3/ho-chi-minh-city.jpg')}} 1024w"
																														sizes="(max-width: 1300px) 100vw, 1300px" />
																												</a>
																											</div>
																											<div
																												class="mkdf-post-info-category mkdf-st-highlight">
																												<a href="category/travel-tips/index.html"
																													rel="category tag">Travel
																													tips</a>
																												<span
																													class="mkdf-st-highlight">
																													<svg version="1.1"
																														xmlns="http://www.w3.org/2000/svg"
																														xmlns:xlink="http://www.w3.org/1999/xlink"
																														x="0px"
																														y="0px"
																														viewBox="0 0 15.7 30"
																														style="enable-background:new 0 0 15.7 30;"
																														xml:space="preserve"
																														class="mkdf-active-hover-left">
																														<polygon
																															class="st0"
																															points="2.6,1 0.7,3.3 2,5.8 2.3,7.6 2.9,8.7 4.4,10.5 3.9,10.8 4.4,11.9 4.4,12.8 4.1,13.8 3.3,14.7 3.9,15.8 4.4,16.8 4,17.5 3.5,18.1 2.2,20.2 3.4,21.5 4.2,24.1 3.4,25.4 2.5,27.4 2.5,27.8 3.2,28.3 4.1,28.5 4.9,29 14.8,29 14.8,1 " />
																													</svg>
																													<span
																														class="mkdf-active-hover-middle"></span>
																													<svg version="1.1"
																														xmlns="http://www.w3.org/2000/svg"
																														xmlns:xlink="http://www.w3.org/1999/xlink"
																														x="0px"
																														y="0px"
																														viewBox="0 0 13.3 30"
																														style="enable-background:new 0 0 13.3 30;"
																														xml:space="preserve"
																														class="mkdf-active-hover-right">
																														<polygon
																															class="st0"
																															points="10,1 10.2,2.1 10.6,2.9 10.6,3.3 10.8,3.7 10.8,4.3 11,5 11,5.7 11,6.3 10.5,6.7 10.8,7.3 11,7.8 	11.6,8.3 11.6,8.6 11.5,8.9 11.6,9.9 11.6,10.5 12.4,11.6 12.1,12 12.4,12.2 11.8,12.8 11.4,13.5 11.6,13.7 11.9,13.7 12,13.9 11.5,15.1 10.8,16 9.1,17.7 9.7,18.2 9.3,19 9.7,19.8 9.6,20.6 9.7,21.5 9.6,21.9 9.6,22.3 10.1,22.8 9.6,23.6 9.7,24 9.7,24.2 9.9,24.4 9.5,24.7 9.3,25.4 9.3,25.9 8.8,26.2 8.5,27.1 8.8,27.8 9.4,28.6 7.8,29 0.9,29 0.9,1 " />
																													</svg>
																												</span>
																											</div>
																										</div>
																										<div
																											class="mkdf-bli-content">
																											<div
																												class="mkdf-bli-info">
																												<div itemprop="dateCreated"
																													class="mkdf-post-info-date entry-date published updated">
																													<span
																														aria-hidden="true"
																														class="mkdf-icon-font-elegant icon_calendar "></span>
																													<a itemprop="url"
																														href="2019/10/index.html">
																														October
																														8,
																														2019
																													</a>
																													<meta
																														itemprop="interactionCount"
																														content="UserComments: 0" />
																												</div>
																												<div
																													class="mkdf-post-info-author">
																													<span
																														aria-hidden="true"
																														class="mkdf-icon-font-elegant icon_pencil "></span>
																													<a itemprop="author"
																														class="mkdf-post-info-author-link"
																														href="author/alisa-michaels/index.html">
																														<span
																															class="mkdf-post-info-author-text">
																															by
																														</span>
																														Alisa
																														Michaels
																													</a>
																												</div>
																											</div>

																											<h5 itemprop="name"
																												class="entry-title mkdf-post-title">
																												<a itemprop="url"
																													href="{{route('blog.blog_dentail',['id'=>1])}}"
																													title="Moving trough: the everyday urban jungle of your city">
																													Moving
																													trough:
																													the
																													everyday
																													urban
																													jungle
																													of
																													your
																													city
																												</a>
																											</h5>
																											<div
																												class="mkdf-bli-excerpt">
																												<div
																													class="mkdf-post-excerpt-holder">
																													<p itemprop="description"
																														class="mkdf-post-excerpt">
																														Lorem
																														ipsum
																														dolor
																														sit
																														amet,
																														consectetur
																														adipisicing
																														elit,
																														sed
																														do
																														eiusmod
																														tempor
																														incididu
																														nt
																														ut
																														labore
																														et
																														dolore
																														magna
																														aliqua.
																														Ut
																														enim.
																														ad
																														minim
																														veniam
																													</p>
																												</div>
																												<div
																													class="mkdf-post-read-more-button">
																													<a itemprop="url"
																														href="{{route('blog.blog_dentail',['id'=>1])}}"
																														target="_self"
																														class="mkdf-btn mkdf-btn-medium mkdf-btn-simple mkdf-btn-svg-icon mkdf-blog-list-button">
																														<span
																															class="mkdf-btn-text">Read
																															More</span>
																														<svg version="1.1"
																															id="Layer_1"
																															xmlns="http://www.w3.org/2000/svg"
																															xmlns:xlink="http://www.w3.org/1999/xlink"
																															x="0px"
																															y="0px"
																															viewBox="0 0 17 17"
																															style="enable-background:new 0 0 17 17;"
																															xml:space="preserve">
																															<g>
																																<path
																																	d="M15,1.9" />
																																<line
																																	x1="1.7"
																																	y1="15.3"
																																	x2="15"
																																	y2="1.9" />
																																<line
																																	x1="16"
																																	y1="1.9"
																																	x2="15"
																																	y2="1.9" />
																																<line
																																	x1="15"
																																	y1="1.9"
																																	x2="1"
																																	y2="1.9" />
																																<path
																																	d="M15,1.9" />
																																<line
																																	x1="15"
																																	y1="16"
																																	x2="15"
																																	y2="1.9" />
																															</g>
																														</svg>
																														<svg version="1.1"
																															id="Layer_1"
																															xmlns="http://www.w3.org/2000/svg"
																															xmlns:xlink="http://www.w3.org/1999/xlink"
																															x="0px"
																															y="0px"
																															viewBox="0 0 17 17"
																															style="enable-background:new 0 0 17 17;"
																															xml:space="preserve">
																															<g>
																																<path
																																	d="M15,1.9" />
																																<line
																																	x1="1.7"
																																	y1="15.3"
																																	x2="15"
																																	y2="1.9" />
																																<line
																																	x1="16"
																																	y1="1.9"
																																	x2="15"
																																	y2="1.9" />
																																<line
																																	x1="15"
																																	y1="1.9"
																																	x2="1"
																																	y2="1.9" />
																																<path
																																	d="M15,1.9" />
																																<line
																																	x1="15"
																																	y1="16"
																																	x2="15"
																																	y2="1.9" />
																															</g>
																														</svg>
																													</a>
																												</div>
																											</div>
																										</div>
																									</div>
																								</li>
																							</ul>
																						</div>
																					</div>
																					<div class="vc_empty_space"
																						style="height: 42px">
																						<span
																							class="vc_empty_space_inner"></span>
																					</div>
																					<div class="mkdf-blog-list-holder mkdf-grid-list mkdf-bl-info-left mkdf-one-columns mkdf-disable-bottom-space mkdf-normal-space mkdf-bl-pag-no-pagination "
																						data-type=info-left
																						data-number-of-posts=1
																						data-number-of-columns=one
																						data-space-between-items=normal
																						data-category=tips
																						data-orderby=date data-order=ASC
																						data-image-size=full
																						data-title-tag=h5
																						data-excerpt-length=47
																						data-post-info-section=yes
																						data-post-info-image=yes
																						data-post-info-author=yes
																						data-post-info-date=yes
																						data-post-info-category=yes
																						data-post-info-comments=no
																						data-post-info-like=no
																						data-post-info-share=no
																						data-pagination-type=no-pagination
																						data-max-num-pages=1
																						data-next-page=2>
																						<div
																							class="mkdf-bl-wrapper mkdf-outer-space">
																							<ul class="mkdf-blog-list">
																								<li
																									class="mkdf-bl-item mkdf-item-space">
																									<div
																										class="mkdf-bli-inner">
																										<div
																											class="mkdf-bli-wrapper">

																											<div
																												class="mkdf-post-image">
																												<a itemprop="url"
																													href="{{route('blog.blog_dentail',['id'=>1])}}"
																													title="Top Hiking Trails in the Peruvian Amazon Rain Forest">
																													<img loading="lazy"
																														decoding="async"
																														width="1300"
																														height="865"
																														src="{{asset('frontend/img/4x3/ho-chi-minh-city-hall.jpg')}}"
																														class="attachment-full size-full wp-post-image"
																														alt="s"
																														srcset="{{asset('frontend/img/4x3/ho-chi-minh-city-hall.jpg')}} 1300w, {{asset('frontend/img/4x3/ho-chi-minh-city-hall.jpg')}} 600w, {{asset('frontend/img/4x3/ho-chi-minh-city-hall.jpg')}} 300w, {{asset('frontend/img/4x3/ho-chi-minh-city-hall.jpg')}} 768w, {{asset('frontend/img/4x3/ho-chi-minh-city-hall.jpg')}} 1024w"
																														sizes="(max-width: 1300px) 100vw, 1300px" />
																												</a>
																											</div>
																											<div
																												class="mkdf-post-info-category mkdf-st-highlight">
																												<a href="category/tips/index.html"
																													rel="category tag">Tips</a>
																												<span
																													class="mkdf-st-highlight">
																													<svg version="1.1"
																														xmlns="http://www.w3.org/2000/svg"
																														xmlns:xlink="http://www.w3.org/1999/xlink"
																														x="0px"
																														y="0px"
																														viewBox="0 0 15.7 30"
																														style="enable-background:new 0 0 15.7 30;"
																														xml:space="preserve"
																														class="mkdf-active-hover-left">
																														<polygon
																															class="st0"
																															points="2.6,1 0.7,3.3 2,5.8 2.3,7.6 2.9,8.7 4.4,10.5 3.9,10.8 4.4,11.9 4.4,12.8 4.1,13.8 3.3,14.7 3.9,15.8 4.4,16.8 4,17.5 3.5,18.1 2.2,20.2 3.4,21.5 4.2,24.1 3.4,25.4 2.5,27.4 2.5,27.8 3.2,28.3 4.1,28.5 4.9,29 14.8,29 14.8,1 " />
																													</svg>
																													<span
																														class="mkdf-active-hover-middle"></span>
																													<svg version="1.1"
																														xmlns="http://www.w3.org/2000/svg"
																														xmlns:xlink="http://www.w3.org/1999/xlink"
																														x="0px"
																														y="0px"
																														viewBox="0 0 13.3 30"
																														style="enable-background:new 0 0 13.3 30;"
																														xml:space="preserve"
																														class="mkdf-active-hover-right">
																														<polygon
																															class="st0"
																															points="10,1 10.2,2.1 10.6,2.9 10.6,3.3 10.8,3.7 10.8,4.3 11,5 11,5.7 11,6.3 10.5,6.7 10.8,7.3 11,7.8 	11.6,8.3 11.6,8.6 11.5,8.9 11.6,9.9 11.6,10.5 12.4,11.6 12.1,12 12.4,12.2 11.8,12.8 11.4,13.5 11.6,13.7 11.9,13.7 12,13.9 11.5,15.1 10.8,16 9.1,17.7 9.7,18.2 9.3,19 9.7,19.8 9.6,20.6 9.7,21.5 9.6,21.9 9.6,22.3 10.1,22.8 9.6,23.6 9.7,24 9.7,24.2 9.9,24.4 9.5,24.7 9.3,25.4 9.3,25.9 8.8,26.2 8.5,27.1 8.8,27.8 9.4,28.6 7.8,29 0.9,29 0.9,1 " />
																													</svg>
																												</span>
																											</div>
																										</div>
																										<div
																											class="mkdf-bli-content">
																											<div
																												class="mkdf-bli-info">
																												<div itemprop="dateCreated"
																													class="mkdf-post-info-date entry-date published updated">
																													<span
																														aria-hidden="true"
																														class="mkdf-icon-font-elegant icon_calendar "></span>
																													<a itemprop="url"
																														href="2019/10/index.html">
																														October
																														10,
																														2019
																													</a>
																													<meta
																														itemprop="interactionCount"
																														content="UserComments: 0" />
																												</div>
																												<div
																													class="mkdf-post-info-author">
																													<span
																														aria-hidden="true"
																														class="mkdf-icon-font-elegant icon_pencil "></span>
																													<a itemprop="author"
																														class="mkdf-post-info-author-link"
																														href="author/alisa-michaels/index.html">
																														<span
																															class="mkdf-post-info-author-text">
																															by
																														</span>
																														Alisa
																														Michaels
																													</a>
																												</div>
																											</div>

																											<h5 itemprop="name"
																												class="entry-title mkdf-post-title">
																												<a itemprop="url"
																													href="{{route('blog.blog_dentail',['id'=>1])}}"
																													title="Top Hiking Trails in the Peruvian Amazon Rain Forest">
																													Top
																													Hiking
																													Trails
																													in
																													the
																													Peruvian
																													Amazon
																													Rain
																													Forest
																												</a>
																											</h5>
																											<div
																												class="mkdf-bli-excerpt">
																												<div
																													class="mkdf-post-excerpt-holder">
																													<p itemprop="description"
																														class="mkdf-post-excerpt">
																														Lorem
																														ipsum
																														dolor
																														sit
																														amet,
																														consectetur
																														adipisicing
																														elit,
																														sed
																														do
																														eiusmod
																														tempor
																														incididu
																														nt
																														ut
																														labore
																														et
																														dolore
																														magna
																														aliqua.
																														Ut
																														enim.
																														ad
																														minim
																														veniam,
																													</p>
																												</div>
																												<div
																													class="mkdf-post-read-more-button">
																													<a itemprop="url"
																														href="{{route('blog.blog_dentail',['id'=>1])}}"
																														target="_self"
																														class="mkdf-btn mkdf-btn-medium mkdf-btn-simple mkdf-btn-svg-icon mkdf-blog-list-button">
																														<span
																															class="mkdf-btn-text">Read
																															More</span>
																														<svg version="1.1"
																															id="Layer_1"
																															xmlns="http://www.w3.org/2000/svg"
																															xmlns:xlink="http://www.w3.org/1999/xlink"
																															x="0px"
																															y="0px"
																															viewBox="0 0 17 17"
																															style="enable-background:new 0 0 17 17;"
																															xml:space="preserve">
																															<g>
																																<path
																																	d="M15,1.9" />
																																<line
																																	x1="1.7"
																																	y1="15.3"
																																	x2="15"
																																	y2="1.9" />
																																<line
																																	x1="16"
																																	y1="1.9"
																																	x2="15"
																																	y2="1.9" />
																																<line
																																	x1="15"
																																	y1="1.9"
																																	x2="1"
																																	y2="1.9" />
																																<path
																																	d="M15,1.9" />
																																<line
																																	x1="15"
																																	y1="16"
																																	x2="15"
																																	y2="1.9" />
																															</g>
																														</svg>
																														<svg version="1.1"
																															id="Layer_1"
																															xmlns="http://www.w3.org/2000/svg"
																															xmlns:xlink="http://www.w3.org/1999/xlink"
																															x="0px"
																															y="0px"
																															viewBox="0 0 17 17"
																															style="enable-background:new 0 0 17 17;"
																															xml:space="preserve">
																															<g>
																																<path
																																	d="M15,1.9" />
																																<line
																																	x1="1.7"
																																	y1="15.3"
																																	x2="15"
																																	y2="1.9" />
																																<line
																																	x1="16"
																																	y1="1.9"
																																	x2="15"
																																	y2="1.9" />
																																<line
																																	x1="15"
																																	y1="1.9"
																																	x2="1"
																																	y2="1.9" />
																																<path
																																	d="M15,1.9" />
																																<line
																																	x1="15"
																																	y1="16"
																																	x2="15"
																																	y2="1.9" />
																															</g>
																														</svg>
																													</a>
																												</div>
																											</div>
																										</div>
																									</div>
																								</li>
																							</ul>
																						</div>
																					</div>
																				</div>
																			</div>
																		</div>
																	</div>
																</div>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
							<div class="vc_row wpb_row vc_row-fluid vc_custom_1579084286868 vc_row-has-fill">
								<div class="wpb_column vc_column_container vc_col-sm-12">
									<div class="vc_column-inner">
										<div class="wpb_wrapper">
											<div
												class="mkdf-elements-holder   mkdf-two-columns  mkdf-responsive-mode-1024 ">
												<div class="mkdf-eh-item    " data-item-class="mkdf-eh-custom-9971"
													data-1400-1600="50px 5% 131px 8%" data-1025-1399="50px 7% 168px 12%"
													data-769-1024="75px 0 70px 16%" data-681-768="75px 12% 70px 14%"
													data-680="110px 6% 30px">
													<div class="mkdf-eh-item-inner">
														<div class="mkdf-eh-item-content mkdf-eh-custom-9971"
															style="padding: 0 30px 120px 24%">
															<div class="mkdf-single-image-holder   ">
																<div class="mkdf-si-inner">
																	<img loading="lazy" decoding="async" width="700"
																		height="512"
																		src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-01.png')}}"
																		class="attachment-full size-full" alt="s"
																		srcset="https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/h1-img-01.png 700w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/h1-img-01-600x439.png 600w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/h1-img-01-300x219.png 300w"
																		sizes="(max-width: 700px) 100vw, 700px" />
																</div>
															</div>
														</div>
													</div>
												</div>
												<div class="mkdf-eh-item    " data-item-class="mkdf-eh-custom-2057"
													data-1400-1600="35px 7% 150px 10px" data-1025-1399="35px 7% 150px 0"
													data-769-1024="0 21% 189px 120px" data-681-768="0 12% 189px 77px"
													data-680="35px 30px 232px 30px">
													<div class="mkdf-eh-item-inner">
														<div class="mkdf-eh-item-content mkdf-eh-custom-2057"
															style="padding: 35px 21% 125px 7px">
															<div class="mkdf-section-title-holder   mkdf-st-highlight"
																style="text-align: left">
																<div class="mkdf-st-inner">
																	<span class="mkdf-st-tagline">Lorem
																		ipsum dolor</span>
																	<h3 class="mkdf-st-title">
																		Finding the perfect trails to hike
																		is easy with <span
																			class="mkdf-st-highlight-title-holder"
																			style="color: #ffffff">
																			<span class="mkdf-st-highlight-title"
																				style="color: #59815b">newsletter</span>
																			<span class="mkdf-st-highlight">
																				<svg version="1.1"
																					xmlns="http://www.w3.org/2000/svg"
																					xmlns:xlink="http://www.w3.org/1999/xlink"
																					x="0px" y="0px"
																					viewBox="0 0 15.7 30"
																					style="enable-background:new 0 0 15.7 30;"
																					xml:space="preserve"
																					class="mkdf-active-hover-left">
																					<polygon class="st0"
																						points="2.6,1 0.7,3.3 2,5.8 2.3,7.6 2.9,8.7 4.4,10.5 3.9,10.8 4.4,11.9 4.4,12.8 4.1,13.8 3.3,14.7 3.9,15.8 4.4,16.8 4,17.5 3.5,18.1 2.2,20.2 3.4,21.5 4.2,24.1 3.4,25.4 2.5,27.4 2.5,27.8 3.2,28.3 4.1,28.5 4.9,29 14.8,29 14.8,1 " />
																				</svg>
																				<span
																					class="mkdf-active-hover-middle"></span>
																				<svg version="1.1"
																					xmlns="http://www.w3.org/2000/svg"
																					xmlns:xlink="http://www.w3.org/1999/xlink"
																					x="0px" y="0px"
																					viewBox="0 0 13.3 30"
																					style="enable-background:new 0 0 13.3 30;"
																					xml:space="preserve"
																					class="mkdf-active-hover-right">
																					<polygon class="st0"
																						points="10,1 10.2,2.1 10.6,2.9 10.6,3.3 10.8,3.7 10.8,4.3 11,5 11,5.7 11,6.3 10.5,6.7 10.8,7.3 11,7.8 	11.6,8.3 11.6,8.6 11.5,8.9 11.6,9.9 11.6,10.5 12.4,11.6 12.1,12 12.4,12.2 11.8,12.8 11.4,13.5 11.6,13.7 11.9,13.7 12,13.9 11.5,15.1 10.8,16 9.1,17.7 9.7,18.2 9.3,19 9.7,19.8 9.6,20.6 9.7,21.5 9.6,21.9 9.6,22.3 10.1,22.8 9.6,23.6 9.7,24 9.7,24.2 9.9,24.4 9.5,24.7 9.3,25.4 9.3,25.9 8.8,26.2 8.5,27.1 8.8,27.8 9.4,28.6 7.8,29 0.9,29 0.9,1 " />
																				</svg>
																			</span>
																		</span> </h3>
																	<p class="mkdf-st-text "
																		style="font-size: 18px;line-height: 26px">
																		Lorem ipsum dolor sit amet,
																		consectetur adipisicing elit, sed do
																		eiusmod tempor incididu nt ut labore
																		et dolore minim veniam, quism. </p>
																</div>
															</div>
															<div class="vc_empty_space" style="height: 32px"><span
																	class="vc_empty_space_inner"></span>
															</div>
															<div class="wpcf7 no-js" id="wpcf7-f89-p29-o1" lang="en-US"
																dir="ltr">
																<div class="screen-reader-response">
																	<p role="status" aria-live="polite"
																		aria-atomic="true"></p>
																	<ul></ul>
																</div>
																<form
																	action="https://wanderland.qodeinteractive.com/#wpcf7-f89-p29-o1"
																	method="post"
																	class="wpcf7-form init cf7_custom_style_3 demo"
																	aria-label="Contact form" novalidate="novalidate"
																	data-status="init">
																	<div style="display: none;">
																		<input type="hidden" name="_wpcf7" value="89" />
																		<input type="hidden" name="_wpcf7_version"
																			value="5.9.2" />
																		<input type="hidden" name="_wpcf7_locale"
																			value="en_US" />
																		<input type="hidden" name="_wpcf7_unit_tag"
																			value="wpcf7-f89-p29-o1" />
																		<input type="hidden"
																			name="_wpcf7_container_post" value="29" />
																		<input type="hidden"
																			name="_wpcf7_posted_data_hash" value="" />
																	</div>
																	<div class="custom-newsletter-2"><span
																			class="wpcf7-form-control-wrap"
																			data-name="your-name"><input size="40"
																				class="wpcf7-form-control wpcf7-text wpcf7-validates-as-required"
																				aria-required="true"
																				aria-invalid="false" placeholder="Name"
																				value="" type="text"
																				name="your-name" /></span><span
																			class="wpcf7-form-control-wrap"
																			data-name="your-email"><input size="40"
																				class="wpcf7-form-control wpcf7-email wpcf7-validates-as-required wpcf7-text wpcf7-validates-as-email"
																				aria-required="true"
																				aria-invalid="false"
																				placeholder="E-mail" value=""
																				type="email"
																				name="your-email" /></span><button
																			class="wpcf7-form-control wpcf7-submit mkdf-btn mkdf-btn-medium mkdf-btn-solid"
																			type="submit"><span
																				class="mkdf-btn-text">subscribe</span></button>
																	</div>
																	<div class="wpcf7-response-output"
																		aria-hidden="true"></div>
																</form>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
							<div class="vc_row wpb_row vc_row-fluid vc_custom_1572593520601 vc_row-has-fill">
								<div class="wpb_column vc_column_container vc_col-sm-12 vc_col-lg-offset-0">
									<div class="vc_column-inner">
										<div class="wpb_wrapper">
											<div class="mkdf-row-grid-section-wrapper ">
												<div class="mkdf-row-grid-section">
													<div class="vc_row wpb_row vc_inner vc_row-fluid">
														<div
															class="wpb_column vc_column_container vc_col-sm-10 vc_col-lg-offset-3 vc_col-lg-6 vc_col-md-offset-1 vc_col-md-10 vc_col-sm-offset-1">
															<div class="vc_column-inner">
																<div class="wpb_wrapper">
																	<div
																		class="mkdf-elements-holder   mkdf-one-column  mkdf-responsive-mode-768 ">
																		<div class="mkdf-eh-item    "
																			data-item-class="mkdf-eh-custom-6708"
																			data-1400-1600="53px 7% 0"
																			data-1025-1399="53px 4% 0"
																			data-769-1024="23px 0 0 0"
																			data-681-768="23px 0 0 0"
																			data-680="23px 0 0 0">
																			<div class="mkdf-eh-item-inner">
																				<div class="mkdf-eh-item-content mkdf-eh-custom-6708"
																					style="padding: 0px 7% 0">
																					<div class="mkdf-section-title-holder   mkdf-st-highlight mkdf-center-align"
																						style="text-align: center">
																						<div class="mkdf-st-inner">
																							<span
																								class="mkdf-st-tagline">Lorem
																								ipsum
																								dolore</span>
																							<h2 class="mkdf-st-title">
																								SunShine <span
																									class="mkdf-st-highlight-title-holder"
																									style="color: #f2f1e7">
																									<span
																										class="mkdf-st-highlight-title"
																										style="color: #59815b">Tours</span>
																									<span
																										class="mkdf-st-highlight">
																										<svg version="1.1"
																											xmlns="http://www.w3.org/2000/svg"
																											xmlns:xlink="http://www.w3.org/1999/xlink"
																											x="0px"
																											y="0px"
																											viewBox="0 0 15.7 30"
																											style="enable-background:new 0 0 15.7 30;"
																											xml:space="preserve"
																											class="mkdf-active-hover-left">
																											<polygon
																												class="st0"
																												points="2.6,1 0.7,3.3 2,5.8 2.3,7.6 2.9,8.7 4.4,10.5 3.9,10.8 4.4,11.9 4.4,12.8 4.1,13.8 3.3,14.7 3.9,15.8 4.4,16.8 4,17.5 3.5,18.1 2.2,20.2 3.4,21.5 4.2,24.1 3.4,25.4 2.5,27.4 2.5,27.8 3.2,28.3 4.1,28.5 4.9,29 14.8,29 14.8,1 " />
																										</svg>
																										<span
																											class="mkdf-active-hover-middle"></span>
																										<svg version="1.1"
																											xmlns="http://www.w3.org/2000/svg"
																											xmlns:xlink="http://www.w3.org/1999/xlink"
																											x="0px"
																											y="0px"
																											viewBox="0 0 13.3 30"
																											style="enable-background:new 0 0 13.3 30;"
																											xml:space="preserve"
																											class="mkdf-active-hover-right">
																											<polygon
																												class="st0"
																												points="10,1 10.2,2.1 10.6,2.9 10.6,3.3 10.8,3.7 10.8,4.3 11,5 11,5.7 11,6.3 10.5,6.7 10.8,7.3 11,7.8 	11.6,8.3 11.6,8.6 11.5,8.9 11.6,9.9 11.6,10.5 12.4,11.6 12.1,12 12.4,12.2 11.8,12.8 11.4,13.5 11.6,13.7 11.9,13.7 12,13.9 11.5,15.1 10.8,16 9.1,17.7 9.7,18.2 9.3,19 9.7,19.8 9.6,20.6 9.7,21.5 9.6,21.9 9.6,22.3 10.1,22.8 9.6,23.6 9.7,24 9.7,24.2 9.9,24.4 9.5,24.7 9.3,25.4 9.3,25.9 8.8,26.2 8.5,27.1 8.8,27.8 9.4,28.6 7.8,29 0.9,29 0.9,1 " />
																										</svg>
																									</span>
																								</span>
																							</h2>
																						</div>
																					</div>
																				</div>
																			</div>
																		</div>
																	</div>
																</div>
															</div>
														</div>
													</div>
												</div>
											</div>
											<div class="mkdf-row-grid-section-wrapper ">
												<div class="mkdf-row-grid-section">
													<div
														class="vc_row wpb_row vc_inner vc_row-fluid vc_custom_1579082282298">
														<div class="wpb_column vc_column_container vc_col-sm-12">
															<div class="vc_column-inner">
																<div class="wpb_wrapper">
																	<div class="mkdf-blog-slider-holder mkdf-bs-slider">
																		<ul class="px-3 mkdf-blog-slider mkdf-owl-slider"
																			data-number-of-items="3"
																			data-slider-margin="22"
																			data-enable-pagination="no">
																			<!-- <li class="mkdf-blog-slider-item">
																				<div class="mkdf-blog-slider-item-inner">
																					<div class="card border-0 shadow-sm">
																						<img src="https://moliva.themenix.com/assets/img/tours/t1.jpg" class="card-img-top" alt="Castle">
																						<div class="card-body">
																							<h5 class="card-title fw-bold">Explore the castle and ancient village in Karda</h5>
																							<p class="card-text text-muted" style="
    font-weight: 400;
">
																								<svg style="width: 1.55rem; height: 1.55rem;" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
																									<path d="M4 10H20V19C20 20.1046 19.1046 21 18 21H6C4.89543 21 4 20.1046 4 19V10Z" stroke="#333333" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
																									<path d="M5.77778 5H4V10H20V5H18.2222M11.1111 5H12.8889" stroke="#333333" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
																									<line x1="8" y1="4" x2="8" y2="6" stroke="#333333" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
																									<line x1="16" y1="4" x2="16" y2="6" stroke="#333333" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
																									<path d="M7 13H7.01" stroke="#333333" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
																									<path d="M7 16H7.01" stroke="#333333" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
																									<path d="M10 13H10.01" stroke="#333333" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
																									<path d="M10 16H10.01" stroke="#333333" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
																									<path d="M13 13H13.01" stroke="#333333" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
																								</svg> 3 days &nbsp;
																								<svg style="width: 1.55rem; height: 1.55rem;" xmlns="http://www.w3.org/2000/svg" id="i-location" viewBox="0 0 32 32" width="32" height="32" fill="none" stroke="currentcolor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
																									<circle cx="16" cy="11" r="4" />
																									<path d="M24 15 C21 22 16 30 16 30 16 30 11 22 8 15 5 8 10 2 16 2 22 2 27 8 24 15 Z" />
																								</svg>3 Destinations
																							</p>
																							<p class="mb-2">
																								<strong>4.9</strong>
																								<span class="text-warning">★★★★★</span>
																								<small class="text-muted">(231 reviews)</small>
																							</p>
																							<p class="price">
																								<span class="fs-4 fw-bold text-success">$195.80</span>
																								<del class="text-muted">$230.00</del>
																							</p>
																							<div class="container text-center
                                                ">
																								<a href="#" class="btn btn-success ">
																									Booking Now <i class="fa-solid fa-arrow-right"></i>
																								</a>
																							</div>

																						</div>
																					</div>
																				</div>
																			</li> -->
																			@if(isset($tours))
																			@foreach ($tours as $tour )
																			<li class="mkdf-blog-slider-item">
																				<div
																					class="mkdf-blog-slider-item-inner">
																					<div class="mkdf-item-image">
																						<a itemprop="url"
																							href="{{route('tour_detail',['id'=>$tour->id])}}">
																							<img loading="lazy"
																								decoding="async"
																								width="1300"
																								height="1551"
																								src="{{ $tour->images->first()->thumbnail }}"
																								class="attachment-full size-full wp-post-image"
																								alt="s" />
																						</a>
																						<div
																							class="mkdf-post-info-category mkdf-st-highlight">
																							<a href="{{route('tour_detail',['id'=>$tour->id])}}"
																								rel="category tag">Service</a>
																							<span
																								class="mkdf-st-highlight">
																								<svg version="1.1"
																									xmlns="http://www.w3.org/2000/svg"
																									xmlns:xlink="http://www.w3.org/1999/xlink"
																									x="0px" y="0px"
																									viewBox="0 0 15.7 30"
																									style="enable-background:new 0 0 15.7 30;"
																									xml:space="preserve"
																									class="mkdf-active-hover-left">
																									<polygon class="st0"
																										points="2.6,1 0.7,3.3 2,5.8 2.3,7.6 2.9,8.7 4.4,10.5 3.9,10.8 4.4,11.9 4.4,12.8 4.1,13.8 3.3,14.7 3.9,15.8 4.4,16.8 4,17.5 3.5,18.1 2.2,20.2 3.4,21.5 4.2,24.1 3.4,25.4 2.5,27.4 2.5,27.8 3.2,28.3 4.1,28.5 4.9,29 14.8,29 14.8,1 " />
																								</svg> <span
																									class="mkdf-active-hover-middle"></span>
																								<svg version="1.1"
																									xmlns="http://www.w3.org/2000/svg"
																									xmlns:xlink="http://www.w3.org/1999/xlink"
																									x="0px" y="0px"
																									viewBox="0 0 13.3 30"
																									style="enable-background:new 0 0 13.3 30;"
																									xml:space="preserve"
																									class="mkdf-active-hover-right">
																									<polygon class="st0"
																										points="10,1 10.2,2.1 10.6,2.9 10.6,3.3 10.8,3.7 10.8,4.3 11,5 11,5.7 11,6.3 10.5,6.7 10.8,7.3 11,7.8 	11.6,8.3 11.6,8.6 11.5,8.9 11.6,9.9 11.6,10.5 12.4,11.6 12.1,12 12.4,12.2 11.8,12.8 11.4,13.5 11.6,13.7 11.9,13.7 12,13.9 11.5,15.1 10.8,16 9.1,17.7 9.7,18.2 9.3,19 9.7,19.8 9.6,20.6 9.7,21.5 9.6,21.9 9.6,22.3 10.1,22.8 9.6,23.6 9.7,24 9.7,24.2 9.9,24.4 9.5,24.7 9.3,25.4 9.3,25.9 8.8,26.2 8.5,27.1 8.8,27.8 9.4,28.6 7.8,29 0.9,29 0.9,1 " />
																								</svg>
																							</span>
																						</div>
																					</div>
																					<div class="mkdf-item-text-wrapper">
																						<div
																							class="mkdf-item-text-holder">
																							<div
																								class="mkdf-item-text-holder-inner">
																								<div
																									class="mkdf-item-info-section">
																									<div itemprop="dateCreated"
																										class="mkdf-post-info-date entry-date published updated">
																										<span
																											aria-hidden="true"
																											class="mkdf-icon-font-elegant icon_calendar "></span>
																										<a itemprop="url"
																											href="2019/10/index.html">
																											October
																											8,
																											2019
																										</a>
																										<meta
																											itemprop="interactionCount"
																											content="UserComments: 0" />
																									</div>
																									<div
																										class="mkdf-post-info-author">
																										<span
																											aria-hidden="true"
																											class="mkdf-icon-font-elegant icon_pencil "></span>
																										<a itemprop="author"
																											class="mkdf-post-info-author-link"
																											href="author/alisa-michaels/index.html">
																											<span
																												class="mkdf-post-info-author-text">
																												by
																											</span>
																											Alisa
																											Michaels
																										</a>
																									</div>
																								</div>

																								<h5 itemprop="name"
																									class="entry-title mkdf-post-title">
																									<a itemprop="url"
																										href="{{route('tour_detail',['id'=>$tour->id])}}"
																										title="The Ultimate Grand Canyon Travel Guide for alpinists">
																										{{ $tour->title }}
																									</a>
																								</h5>
																								<p itemprop="description" class="mkdf-post-excerpt" style ="font-weight:400">
																									{{ Str::limit(strip_tags($tour->description), 80, '...') }}
																													</p>
																							</div>
																						</div>
																					</div>
																				</div>
																			</li>
																			@endforeach
																			@endif


																		</ul>
																	</div>
																</div>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
							<div class="vc_row wpb_row vc_row-fluid vc_custom_1579108147673 vc_row-has-fill">
								<div class="wpb_column vc_column_container vc_col-sm-12">
									<div class="vc_column-inner">
										<div class="wpb_wrapper"></div>
									</div>
								</div>
							</div>
							<div class="vc_row wpb_row vc_row-fluid vc_custom_1579107555274 vc_row-has-fill"
								style="background-color:#f1f1e7;padding-bottom:0 !important">
								<div class="wpb_column vc_column_container vc_col-sm-12">
									<div class="vc_column-inner">
										<div class="wpb_wrapper">
											<div class="vc_row wpb_row vc_inner vc_row-fluid vc_custom_1579085340244" style="padding-bottom:0 !important">
												<div
													class="wpb_column vc_column_container vc_col-sm-12 vc_col-lg-offset-3 vc_col-lg-6">
													<div class="vc_column-inner">
														<div class="wpb_wrapper">
															<div class="mkdf-section-title-holder   mkdf-st-highlight mkdf-center-align"
																style="text-align: center">
																<div class="mkdf-st-inner">
																	<span class="mkdf-st-tagline">Lorem
																		ipsum dolor</span>
																	<h2 class="mkdf-st-title">
																		Customer <span
																			class="mkdf-st-highlight-title-holder"
																			style="color: #ffffff">
																			<span class="mkdf-st-highlight-title"
																				style="color: #59815b"> Feedback </span>
																			<span class="mkdf-st-highlight">
																				<svg version="1.1"
																					xmlns="http://www.w3.org/2000/svg"
																					xmlns:xlink="http://www.w3.org/1999/xlink"
																					x="0px" y="0px"
																					viewBox="0 0 15.7 30"
																					style="enable-background:new 0 0 15.7 30;"
																					xml:space="preserve"
																					class="mkdf-active-hover-left">
																					<polygon class="st0"
																						points="2.6,1 0.7,3.3 2,5.8 2.3,7.6 2.9,8.7 4.4,10.5 3.9,10.8 4.4,11.9 4.4,12.8 4.1,13.8 3.3,14.7 3.9,15.8 4.4,16.8 4,17.5 3.5,18.1 2.2,20.2 3.4,21.5 4.2,24.1 3.4,25.4 2.5,27.4 2.5,27.8 3.2,28.3 4.1,28.5 4.9,29 14.8,29 14.8,1 " />
																				</svg>
																				<span
																					class="mkdf-active-hover-middle"></span>
																				<svg version="1.1"
																					xmlns="http://www.w3.org/2000/svg"
																					xmlns:xlink="http://www.w3.org/1999/xlink"
																					x="0px" y="0px"
																					viewBox="0 0 13.3 30"
																					style="enable-background:new 0 0 13.3 30;"
																					xml:space="preserve"
																					class="mkdf-active-hover-right">
																					<polygon class="st0"
																						points="10,1 10.2,2.1 10.6,2.9 10.6,3.3 10.8,3.7 10.8,4.3 11,5 11,5.7 11,6.3 10.5,6.7 10.8,7.3 11,7.8 	11.6,8.3 11.6,8.6 11.5,8.9 11.6,9.9 11.6,10.5 12.4,11.6 12.1,12 12.4,12.2 11.8,12.8 11.4,13.5 11.6,13.7 11.9,13.7 12,13.9 11.5,15.1 10.8,16 9.1,17.7 9.7,18.2 9.3,19 9.7,19.8 9.6,20.6 9.7,21.5 9.6,21.9 9.6,22.3 10.1,22.8 9.6,23.6 9.7,24 9.7,24.2 9.9,24.4 9.5,24.7 9.3,25.4 9.3,25.9 8.8,26.2 8.5,27.1 8.8,27.8 9.4,28.6 7.8,29 0.9,29 0.9,1 " />
																				</svg>
																			</span>
																		</span> </h2>
																</div>
															</div>
														</div>
													</div>
												</div>
											</div>
											<!-- <div class="mkdf-row-grid-section-wrapper ">
												<div class="mkdf-row-grid-section">
													<div class="vc_row wpb_row vc_inner vc_row-fluid">
														<div class="wpb_column vc_column_container vc_col-sm-12">
															<div class="vc_column-inner">
																<div class="wpb_wrapper">
																	<div
																		class="mkdf-pl-holder mkdf-grid-list mkdf-disable-bottom-space mkdf-standard-layout mkdf-five-columns mkdf-no-space mkdf-info-below-image ">
																		<div class="mkdf-pl-outer mkdf-outer-space">
																			<div class="mkdf-pli mkdf-item-space ">
																				<div class="mkdf-pli-inner">
																					<div class="mkdf-pli-image">

																						<div class="mkdf-mark-wrapper">

																						</div>
																						<img loading="lazy"
																							decoding="async" width="600"
																							height="502"
																							src="{{asset('frontend/wp-content/uploads/2019/10/shop-img-10-1-600x502.jpg')}}"
																							class="attachment-woocommerce_single size-woocommerce_single wp-post-image"
																							alt="s"
																							srcset="https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-10-1-600x502.jpg 600w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-10-1-300x251.jpg 300w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-10-1-768x642.jpg 768w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-10-1.jpg 800w"
																							sizes="(max-width: 600px) 100vw, 600px" />
																						<img decoding="async"
																							class="mkdf-product-hover-image"
																							alt="a"
																							src="{{asset('frontend/wp-content/uploads/2019/10/shop-img-18.png')}}" />
																					</div>
																					<div class="mkdf-pli-text">
																						<div
																							class="mkdf-pli-text-outer">
																							<div
																								class="mkdf-pli-text-inner">

																								<div
																									class="mkdf-pli-add-to-cart mkdf-default-skin">
																									<a rel="nofollow"
																										href="index71f3.html?add-to-cart=994"
																										data-quantity="1"
																										data-product_id="994"
																										data-product_sku="456"
																										class="button add_to_cart_button ajax_add_to_cart mkdf-button">Add
																										to
																										cart</a>
																								</div>
																							</div>
																						</div>
																					</div>
																					<a class="mkdf-pli-link"
																						itemprop="url"
																						href="product/shoes/index.html"
																						title="Shoes"></a>
																				</div>
																				<div class="mkdf-pli-text-wrapper">
																					<div class="mkdf-pli-price">
																						<span
																							class="woocommerce-Price-amount amount"><span
																								class="woocommerce-Price-currencySymbol">&#036;</span>250.00</span>
																					</div>

																					<h6 itemprop="name"
																						class="entry-title mkdf-pli-title">
																						<a itemprop="url"
																							href="product/shoes/index.html">Shoes</a>
																					</h6>




																					<div class="mkdf-pli-rating-holder">
																						<div class="mkdf-pli-rating"
																							title="Rated %s out of 5">
																							<span
																								style="width: 100%"></span>
																						</div>
																					</div>
																				</div>
																			</div>
																			<div class="mkdf-pli mkdf-item-space ">
																				<div class="mkdf-pli-inner">
																					<div class="mkdf-pli-image">

																						<div class="mkdf-mark-wrapper">

																						</div>
																						<img loading="lazy"
																							decoding="async" width="600"
																							height="502"
																							src="{{asset('frontend/wp-content/uploads/2019/10/shop-img-12-1-600x502.jpg')}}"
																							class="attachment-woocommerce_single size-woocommerce_single wp-post-image"
																							alt="s"
																							srcset="https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-12-1-600x502.jpg 600w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-12-1-300x251.jpg 300w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-12-1-768x642.jpg 768w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-12-1.jpg 800w"
																							sizes="(max-width: 600px) 100vw, 600px" />
																						<img decoding="async"
																							class="mkdf-product-hover-image"
																							alt="s"
																							src="{{asset('frontend/wp-content/uploads/2019/10/shop-img-17.png')}}" />
																					</div>
																					<div class="mkdf-pli-text">
																						<div
																							class="mkdf-pli-text-outer">
																							<div
																								class="mkdf-pli-text-inner">

																								<div
																									class="mkdf-pli-add-to-cart mkdf-default-skin">
																									<a rel="nofollow"
																										href="index0617.html?add-to-cart=1013"
																										data-quantity="1"
																										data-product_id="1013"
																										data-product_sku="41-1"
																										class="button add_to_cart_button ajax_add_to_cart mkdf-button">Add
																										to
																										cart</a>
																								</div>
																							</div>
																						</div>
																					</div>
																					<a class="mkdf-pli-link"
																						itemprop="url"
																						href="product/camera-tripod-2/index.html"
																						title="Camera tripod"></a>
																				</div>
																				<div class="mkdf-pli-text-wrapper">
																					<div class="mkdf-pli-price">
																						<span
																							class="woocommerce-Price-amount amount"><span
																								class="woocommerce-Price-currencySymbol">&#036;</span>200.00</span>
																					</div>

																					<h6 itemprop="name"
																						class="entry-title mkdf-pli-title">
																						<a itemprop="url"
																							href="product/camera-tripod-2/index.html">Camera
																							tripod</a>
																					</h6>




																					<div class="mkdf-pli-rating-holder">
																						<div class="mkdf-pli-rating"
																							title="Rated %s out of 5">
																							<span
																								style="width: 100%"></span>
																						</div>
																					</div>
																				</div>
																			</div>
																			<div class="mkdf-pli mkdf-item-space ">
																				<div class="mkdf-pli-inner">
																					<div class="mkdf-pli-image">

																						<div class="mkdf-mark-wrapper">

																						</div>
																						<img loading="lazy"
																							decoding="async" width="600"
																							height="502"
																							src="{{asset('frontend/wp-content/uploads/2019/10/shop-img-11w-600x502.jpg')}}"
																							class="attachment-woocommerce_single size-woocommerce_single wp-post-image"
																							alt="s"
																							srcset="https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-11w-600x502.jpg 600w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-11w-300x251.jpg 300w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-11w-768x642.jpg 768w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-11w.jpg 800w"
																							sizes="(max-width: 600px) 100vw, 600px" />
																						<img decoding="async"
																							class="mkdf-product-hover-image"
																							alt="s"
																							src="{{asset('frontend/wp-content/uploads/2019/10/test-img-02.png')}}" />
																					</div>
																					<div class="mkdf-pli-text">
																						<div
																							class="mkdf-pli-text-outer">
																							<div
																								class="mkdf-pli-text-inner">

																								<div
																									class="mkdf-pli-add-to-cart mkdf-default-skin">
																									<a rel="nofollow"
																										href="indexf0b3.html?add-to-cart=1006"
																										data-quantity="1"
																										data-product_id="1006"
																										data-product_sku="37"
																										class="button add_to_cart_button ajax_add_to_cart mkdf-button">Add
																										to
																										cart</a>
																								</div>
																							</div>
																						</div>
																					</div>
																					<a class="mkdf-pli-link"
																						itemprop="url"
																						href="product/raincoat-2/index.html"
																						title="Raincoat"></a>
																				</div>
																				<div class="mkdf-pli-text-wrapper">
																					<div class="mkdf-pli-price">
																						<span
																							class="woocommerce-Price-amount amount"><span
																								class="woocommerce-Price-currencySymbol">&#036;</span>200.00</span>
																					</div>

																					<h6 itemprop="name"
																						class="entry-title mkdf-pli-title">
																						<a itemprop="url"
																							href="product/raincoat-2/index.html">Raincoat</a>
																					</h6>




																					<div class="mkdf-pli-rating-holder">
																						<div class="mkdf-pli-rating"
																							title="Rated %s out of 5">
																							<span
																								style="width: 100%"></span>
																						</div>
																					</div>
																				</div>
																			</div>
																			<div class="mkdf-pli mkdf-item-space ">
																				<div class="mkdf-pli-inner">
																					<div class="mkdf-pli-image">

																						<div class="mkdf-mark-wrapper">

																						</div>
																						<img loading="lazy"
																							decoding="async" width="600"
																							height="502"
																							src="{{asset('frontend/wp-content/uploads/2019/10/shop-img-13-1-600x502.jpg')}}"
																							class="attachment-woocommerce_single size-woocommerce_single wp-post-image"
																							alt="s"
																							srcset="https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-13-1-600x502.jpg 600w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-13-1-300x251.jpg 300w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-13-1-768x642.jpg 768w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-13-1.jpg 800w"
																							sizes="(max-width: 600px) 100vw, 600px" />
																						<img decoding="async"
																							class="mkdf-product-hover-image"
																							alt="a"
																							src="{{asset('frontend/wp-content/uploads/2019/10/shop-img-15.png')}}" />
																					</div>
																					<div class="mkdf-pli-text">
																						<div
																							class="mkdf-pli-text-outer">
																							<div
																								class="mkdf-pli-text-inner">

																								<div
																									class="mkdf-pli-add-to-cart mkdf-default-skin">
																									<a rel="nofollow"
																										href="indexd518.html?add-to-cart=1020"
																										data-quantity="1"
																										data-product_id="1020"
																										data-product_sku="94"
																										class="button add_to_cart_button ajax_add_to_cart mkdf-button">Add
																										to
																										cart</a>
																								</div>
																							</div>
																						</div>
																					</div>
																					<a class="mkdf-pli-link"
																						itemprop="url"
																						href="product/dslr-camera-2/index.html"
																						title="DSLR camera"></a>
																				</div>
																				<div class="mkdf-pli-text-wrapper">
																					<div class="mkdf-pli-price">
																						<span
																							class="woocommerce-Price-amount amount"><span
																								class="woocommerce-Price-currencySymbol">&#036;</span>1,800.00</span>
																					</div>

																					<h6 itemprop="name"
																						class="entry-title mkdf-pli-title">
																						<a itemprop="url"
																							href="product/dslr-camera-2/index.html">DSLR
																							camera</a>
																					</h6>




																					<div class="mkdf-pli-rating-holder">
																						<div class="mkdf-pli-rating"
																							title="Rated %s out of 5">
																							<span
																								style="width: 0%"></span>
																						</div>
																					</div>
																				</div>
																			</div>
																			<div class="mkdf-pli mkdf-item-space ">
																				<div class="mkdf-pli-inner">
																					<div class="mkdf-pli-image">

																						<div class="mkdf-mark-wrapper">

																						</div>
																						<img loading="lazy"
																							decoding="async" width="600"
																							height="502"
																							src="{{asset('frontend/wp-content/uploads/2019/10/shop-img-14-1-600x502.jpg')}}"
																							class="attachment-woocommerce_single size-woocommerce_single wp-post-image"
																							alt="f"
																							srcset="https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-14-1-600x502.jpg 600w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-14-1-300x251.jpg 300w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-14-1-768x642.jpg 768w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/shop-img-14-1.jpg 800w"
																							sizes="(max-width: 600px) 100vw, 600px" />
																						<img decoding="async"
																							class="mkdf-product-hover-image"
																							alt="s"
																							src="{{asset('frontend/wp-content/uploads/2019/10/shop-img-16.png')}}" />
																					</div>
																					<div class="mkdf-pli-text">
																						<div
																							class="mkdf-pli-text-outer">
																							<div
																								class="mkdf-pli-text-inner">

																								<div
																									class="mkdf-pli-add-to-cart mkdf-default-skin">
																									<a rel="nofollow"
																										href="index603e.html?add-to-cart=1028"
																										data-quantity="1"
																										data-product_id="1028"
																										data-product_sku="99"
																										class="button add_to_cart_button ajax_add_to_cart mkdf-button">Add
																										to
																										cart</a>
																								</div>
																							</div>
																						</div>
																					</div>
																					<a class="mkdf-pli-link"
																						itemprop="url"
																						href="product/watter-bottle-2/index.html"
																						title="Watter bottle"></a>
																				</div>
																				<div class="mkdf-pli-text-wrapper">
																					<div class="mkdf-pli-price">
																						<span
																							class="woocommerce-Price-amount amount"><span
																								class="woocommerce-Price-currencySymbol">&#036;</span>17.00</span>
																					</div>

																					<h6 itemprop="name"
																						class="entry-title mkdf-pli-title">
																						<a itemprop="url"
																							href="product/watter-bottle-2/index.html">Watter
																							bottle</a>
																					</h6>




																					<div class="mkdf-pli-rating-holder">
																						<div class="mkdf-pli-rating"
																							title="Rated %s out of 5">
																							<span
																								style="width: 0%"></span>
																						</div>
																					</div>
																				</div>
																			</div>
																		</div>
																	</div>
																</div>
															</div>
														</div>
													</div>
												</div>
											</div> -->
										</div>
									</div>
								</div>
							</div>
							<div class="vc_row wpb_row vc_row-fluid vc_custom_1580825328008 vc_row-has-fill">
								<div class="wpb_column vc_column_container vc_col-sm-12">
									<div class="vc_column-inner">
										<div class="wpb_wrapper">
											<div class="mkdf-row-grid-section-wrapper ">
												<div class="mkdf-row-grid-section">
													<div class="vc_row wpb_row vc_inner vc_row-fluid">
														<div class="wpb_column vc_column_container vc_col-sm-12">
															<div class="vc_column-inner">
																<div class="wpb_wrapper">
																	<div
																		class="mkdf-elements-holder   mkdf-one-column  mkdf-responsive-mode-768 ">
																		<div class="mkdf-eh-item    "
																			data-item-class="mkdf-eh-custom-6459"
																			data-1400-1600="0 0 175px"
																			data-1025-1399="0 0 175px"
																			data-769-1024="0 0 145px"
																			data-681-768="0 0 145px"
																			data-680="0 0 108px">
																			<div class="mkdf-eh-item-inner">
																				<div class="mkdf-eh-item-content mkdf-eh-custom-6459"
																					style="padding: 0 0 175px">
																					<div
																						class="mkdf-destination-slider-holder">
																						<div class="mkdf-destination-list-holder mkdf-grid-list mkdf-one-columns mkdf-huge-space mkdf-disable-bottom-space  mkdf-dl-pag-no-pagination    mkdf-pag-below-slider"
																							data-number-of-columns=one
																							data-space-between-items=huge
																							data-number-of-items=9
																							data-image-proportions=full
																							data-enable-image-shadow=no
																							data-category=popular
																							data-orderby=date
																							data-order=ASC
																							data-enable-title=yes
																							data-title-tag=h4
																							data-title-highlight=yes
																							data-enable-category=yes
																							data-enable-excerpt=yes
																							data-excerpt-length=90
																							data-pagination-type=no-pagination
																							data-enable-article-animation=no
																							data-destination-slider-on=yes
																							data-enable-loop=yes
																							data-enable-autoplay=yes
																							data-slider-speed=5000
																							data-slider-speed-animation=600
																							data-enable-navigation=yes
																							data-enable-pagination=no
																							data-pagination-position=below-slider
																							data-enable-button=yes
																							data-max-num-pages=1
																							data-next-page=2>
																							<div
																								class="mkdf-pl-inner mkdf-outer-space mkdf-owl-slider mkdf-list-is-slider clearfix">
																								<article
																									class="mkdf-dl-item mkdf-item-space">
																									<div
																										class="mkdf-dl-item-inner">
																										<div
																											class="mkdf-dli-image d-flex justify-content-center">
																											<div class="col-12 col-md-7 col-lg-8">
																												<img loading="lazy"
																													decoding="async"
																													width="1299"
																													height="714"

																													src="{{asset('frontend\img\feedback\feedback_01.jpg')}}"
																													class="rounded attachment-full size-full wp-post-image"
																													alt="a"
																													srcset="{{asset('frontend\img\feedback\feedback_01.jpg')}} 1299w, {{asset('frontend\img\feedback\feedback_01.jpg')}} 300w, {{asset('frontend\img\feedback\feedback_01.jpg')}} 768w, {{asset('frontend\img\feedback\feedback_01.jpg')}} 1024w, {{asset('frontend\img\feedback\feedback_01.jpg')}} 600w"
																													sizes="(max-width: 1299px) 100vw, 1299px" />
																											</div>
																										</div>
																										<div
																											class="mkdf-dli-text-holder">
																											<div
																												class="mkdf-dli-text-wrapper">
																												<div
																													class="mkdf-dli-text">
																													<div
																														class="mkdf-dli-text-inner">
																														<svg xmlns="http://www.w3.org/2000/svg"
																															xmlns:xlink="http://www.w3.org/1999/xlink"
																															x="0px"
																															y="0px"
																															viewBox="0 0 14.1 28"
																															style="enable-background:new 0 0 14.1 28;"
																															xml:space="preserve"
																															class="mkdf-active-hover-left">
																															<polygon
																																class="st0"
																																points="1.9,0 0,2.3 1.3,4.8 1.6,6.6 2.2,7.7 3.7,9.5 3.2,9.8 3.7,10.9 3.7,11.8 3.4,12.8 2.6,13.7 3.2,14.8
3.7,15.8 3.3,16.5 2.8,17.1 1.5,19.2 2.7,20.5 3.5,23.1 2.7,24.4 1.8,26.4 1.8,26.8 2.5,27.3 3.4,27.5 4.2,28 14.1,28 14.1,0 " />
																														</svg>
																														<svg xmlns="http://www.w3.org/2000/svg"
																															xmlns:xlink="http://www.w3.org/1999/xlink"
																															x="0px"
																															y="0px"
																															viewBox="0 0 11.5 28"
																															style="enable-background:new 0 0 11.5 28;"
																															xml:space="preserve"
																															class="mkdf-active-hover-right">
																															<polygon
																																class="st0"
																																points="9.1,0 9.3,1.1 9.7,1.9 9.7,2.3 9.9,2.7 9.9,3.3 10.1,4 10.1,4.7 10.1,5.3 9.6,5.7 9.9,6.3 10.1,6.8
10.7,7.3 10.7,7.6 10.6,7.9 10.7,8.9 10.7,9.5 11.5,10.6 11.2,11 11.5,11.2 10.9,11.8 10.5,12.5 10.7,12.7 11,12.7 11.1,12.9
10.6,14.1 9.9,15 8.2,16.7 8.8,17.2 8.4,18 8.8,18.8 8.7,19.6 8.8,20.5 8.7,20.9 8.7,21.3 9.2,21.8 8.7,22.6 8.8,23 8.8,23.2
9,23.4 8.6,23.7 8.4,24.4 8.4,24.9 7.9,25.2 7.6,26.1 7.9,26.8 8.5,27.6 6.9,28 0,28 0,0 " />
																														</svg>
																														<div
																															class="mkdf-dli-category-holder">
																															<a itemprop="url"
																																class="mkdf-dli-category"
																																href="destination-category/popular/index.html">Popular</a>
																														</div>
																														<h4 itemprop="name"
																															class="mkdf-dli-title entry-title">
																															<span>
																																<span
																																	class="mkdf-dli-title-wrap">
																																	Sweden
																																</span>
																															</span>
																														</h4>
																														<p itemprop="description"
																															class="mkdf-dli-excerpt">
																															Lorem
																															ipsum
																															dolor
																															sit
																															amet,
																															consectetur
																															adipisicing
																															elit,
																															sed
																															do
																															eiusmod
																															tempor
																														</p>
																														<div
																															class="mkdf-dli-btn-holder">
																															<a itemprop="url"
																																href="destination-item/sweden/index.html"
																																target="_self"
																																class="mkdf-btn mkdf-btn-medium mkdf-btn-simple mkdf-btn-svg-icon">
																																<span
																																	class="mkdf-btn-text">Read
																																	More</span>
																																<svg version="1.1"
																																	id="Layer_1"
																																	xmlns="http://www.w3.org/2000/svg"
																																	xmlns:xlink="http://www.w3.org/1999/xlink"
																																	x="0px"
																																	y="0px"
																																	viewBox="0 0 17 17"
																																	style="enable-background:new 0 0 17 17;"
																																	xml:space="preserve">
																																	<g>
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="1.7"
																																			y1="15.3"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="16"
																																			y1="1.9"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="15"
																																			y1="1.9"
																																			x2="1"
																																			y2="1.9" />
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="15"
																																			y1="16"
																																			x2="15"
																																			y2="1.9" />
																																	</g>
																																</svg>
																																<svg version="1.1"
																																	id="Layer_1"
																																	xmlns="http://www.w3.org/2000/svg"
																																	xmlns:xlink="http://www.w3.org/1999/xlink"
																																	x="0px"
																																	y="0px"
																																	viewBox="0 0 17 17"
																																	style="enable-background:new 0 0 17 17;"
																																	xml:space="preserve">
																																	<g>
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="1.7"
																																			y1="15.3"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="16"
																																			y1="1.9"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="15"
																																			y1="1.9"
																																			x2="1"
																																			y2="1.9" />
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="15"
																																			y1="16"
																																			x2="15"
																																			y2="1.9" />
																																	</g>
																																</svg>
																															</a>
																														</div>
																													</div>
																												</div>
																											</div>
																										</div>
																										<a itemprop="url"
																											class="mkdf-dli-link mkdf-block-drag-link"
																											href="destination-item/sweden/index.html"></a>
																									</div>
																								</article>
																								<article
																									class="mkdf-dl-item mkdf-item-space">
																									<div
																										class="mkdf-dl-item-inner">
																										<div
																											class="mkdf-dli-image d-flex justify-content-center">
																											<div class="col-12 col-md-7 col-lg-8">
																												<img loading="lazy"
																													decoding="async"
																													width="1299"
																													height="714"

																													src="{{asset('frontend\img\feedback\feedback_02.jpg')}}"
																													class="rounded attachment-full size-full wp-post-image"
																													alt="a"
																													srcset="{{asset('frontend\img\feedback\feedback_02.jpg')}} 1299w, {{asset('frontend\img\feedback\feedback_02.jpg')}} 300w, {{asset('frontend\img\feedback\feedback_02.jpg')}} 768w, {{asset('frontend\img\feedback\feedback_02.jpg')}} 1024w, {{asset('frontend\img\feedback\feedback_02.jpg')}} 600w"
																													sizes="(max-width: 1299px) 100vw, 1299px" />
																											</div>
																										</div>
																										<div
																											class="mkdf-dli-text-holder">
																											<div
																												class="mkdf-dli-text-wrapper">
																												<div
																													class="mkdf-dli-text">
																													<div
																														class="mkdf-dli-text-inner">
																														<svg xmlns="http://www.w3.org/2000/svg"
																															xmlns:xlink="http://www.w3.org/1999/xlink"
																															x="0px"
																															y="0px"
																															viewBox="0 0 14.1 28"
																															style="enable-background:new 0 0 14.1 28;"
																															xml:space="preserve"
																															class="mkdf-active-hover-left">
																															<polygon
																																class="st0"
																																points="1.9,0 0,2.3 1.3,4.8 1.6,6.6 2.2,7.7 3.7,9.5 3.2,9.8 3.7,10.9 3.7,11.8 3.4,12.8 2.6,13.7 3.2,14.8
3.7,15.8 3.3,16.5 2.8,17.1 1.5,19.2 2.7,20.5 3.5,23.1 2.7,24.4 1.8,26.4 1.8,26.8 2.5,27.3 3.4,27.5 4.2,28 14.1,28 14.1,0 " />
																														</svg>
																														<svg xmlns="http://www.w3.org/2000/svg"
																															xmlns:xlink="http://www.w3.org/1999/xlink"
																															x="0px"
																															y="0px"
																															viewBox="0 0 11.5 28"
																															style="enable-background:new 0 0 11.5 28;"
																															xml:space="preserve"
																															class="mkdf-active-hover-right">
																															<polygon
																																class="st0"
																																points="9.1,0 9.3,1.1 9.7,1.9 9.7,2.3 9.9,2.7 9.9,3.3 10.1,4 10.1,4.7 10.1,5.3 9.6,5.7 9.9,6.3 10.1,6.8
10.7,7.3 10.7,7.6 10.6,7.9 10.7,8.9 10.7,9.5 11.5,10.6 11.2,11 11.5,11.2 10.9,11.8 10.5,12.5 10.7,12.7 11,12.7 11.1,12.9
10.6,14.1 9.9,15 8.2,16.7 8.8,17.2 8.4,18 8.8,18.8 8.7,19.6 8.8,20.5 8.7,20.9 8.7,21.3 9.2,21.8 8.7,22.6 8.8,23 8.8,23.2
9,23.4 8.6,23.7 8.4,24.4 8.4,24.9 7.9,25.2 7.6,26.1 7.9,26.8 8.5,27.6 6.9,28 0,28 0,0 " />
																														</svg>
																														<div
																															class="mkdf-dli-category-holder">
																															<a itemprop="url"
																																class="mkdf-dli-category"
																																href="destination-category/popular/index.html">Popular</a>
																														</div>
																														<h4 itemprop="name"
																															class="mkdf-dli-title entry-title">
																															<span>
																																<span
																																	class="mkdf-dli-title-wrap">
																																	Stockholm
																																</span>
																															</span>
																														</h4>
																														<p itemprop="description"
																															class="mkdf-dli-excerpt">
																															Lorem
																															ipsum
																															dolor
																															sit
																															amet,
																															consectetur
																															adipisicing
																															elit,
																															sed
																															do
																															eiusmod
																															tempor
																														</p>
																														<div
																															class="mkdf-dli-btn-holder">
																															<a itemprop="url"
																																href="destination-item/stockholm/index.html"
																																target="_self"
																																class="mkdf-btn mkdf-btn-medium mkdf-btn-simple mkdf-btn-svg-icon">
																																<span
																																	class="mkdf-btn-text">Read
																																	More</span>
																																<svg version="1.1"
																																	id="Layer_1"
																																	xmlns="http://www.w3.org/2000/svg"
																																	xmlns:xlink="http://www.w3.org/1999/xlink"
																																	x="0px"
																																	y="0px"
																																	viewBox="0 0 17 17"
																																	style="enable-background:new 0 0 17 17;"
																																	xml:space="preserve">
																																	<g>
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="1.7"
																																			y1="15.3"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="16"
																																			y1="1.9"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="15"
																																			y1="1.9"
																																			x2="1"
																																			y2="1.9" />
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="15"
																																			y1="16"
																																			x2="15"
																																			y2="1.9" />
																																	</g>
																																</svg>
																																<svg version="1.1"
																																	id="Layer_1"
																																	xmlns="http://www.w3.org/2000/svg"
																																	xmlns:xlink="http://www.w3.org/1999/xlink"
																																	x="0px"
																																	y="0px"
																																	viewBox="0 0 17 17"
																																	style="enable-background:new 0 0 17 17;"
																																	xml:space="preserve">
																																	<g>
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="1.7"
																																			y1="15.3"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="16"
																																			y1="1.9"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="15"
																																			y1="1.9"
																																			x2="1"
																																			y2="1.9" />
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="15"
																																			y1="16"
																																			x2="15"
																																			y2="1.9" />
																																	</g>
																																</svg>
																															</a>
																														</div>
																													</div>
																												</div>
																											</div>
																										</div>
																										<a itemprop="url"
																											class="mkdf-dli-link mkdf-block-drag-link"
																											href="destination-item/stockholm/index.html"></a>
																									</div>
																								</article>
																								<article
																									class="mkdf-dl-item mkdf-item-space">
																									<div
																										class="mkdf-dl-item-inner">
																										<div
																											class="mkdf-dli-image d-flex justify-content-center">
																											<div class="col-12 col-md-7 col-lg-8">
																												<img loading="lazy"
																													decoding="async"
																													width="1299"
																													height="714"

																													src="{{asset('frontend\img\feedback\feedback_03.JPG')}}"
																													class="rounded attachment-full size-full wp-post-image"
																													alt="a"
																													srcset="{{asset('frontend\img\feedback\feedback_03.JPG')}} 1299w, {{asset('frontend\img\feedback\feedback_03.JPG')}} 300w, {{asset('frontend\img\feedback\feedback_03.JPG')}} 768w, {{asset('frontend\img\feedback\feedback_03.JPG')}} 1024w, {{asset('frontend\img\feedback\feedback_03.JPG')}} 600w"
																													sizes="(max-width: 1299px) 100vw, 1299px" />
																											</div>
																										</div>
																										<div
																											class="mkdf-dli-text-holder">
																											<div
																												class="mkdf-dli-text-wrapper">
																												<div
																													class="mkdf-dli-text">
																													<div
																														class="mkdf-dli-text-inner">
																														<svg xmlns="http://www.w3.org/2000/svg"
																															xmlns:xlink="http://www.w3.org/1999/xlink"
																															x="0px"
																															y="0px"
																															viewBox="0 0 14.1 28"
																															style="enable-background:new 0 0 14.1 28;"
																															xml:space="preserve"
																															class="mkdf-active-hover-left">
																															<polygon
																																class="st0"
																																points="1.9,0 0,2.3 1.3,4.8 1.6,6.6 2.2,7.7 3.7,9.5 3.2,9.8 3.7,10.9 3.7,11.8 3.4,12.8 2.6,13.7 3.2,14.8
3.7,15.8 3.3,16.5 2.8,17.1 1.5,19.2 2.7,20.5 3.5,23.1 2.7,24.4 1.8,26.4 1.8,26.8 2.5,27.3 3.4,27.5 4.2,28 14.1,28 14.1,0 " />
																														</svg>
																														<svg xmlns="http://www.w3.org/2000/svg"
																															xmlns:xlink="http://www.w3.org/1999/xlink"
																															x="0px"
																															y="0px"
																															viewBox="0 0 11.5 28"
																															style="enable-background:new 0 0 11.5 28;"
																															xml:space="preserve"
																															class="mkdf-active-hover-right">
																															<polygon
																																class="st0"
																																points="9.1,0 9.3,1.1 9.7,1.9 9.7,2.3 9.9,2.7 9.9,3.3 10.1,4 10.1,4.7 10.1,5.3 9.6,5.7 9.9,6.3 10.1,6.8
10.7,7.3 10.7,7.6 10.6,7.9 10.7,8.9 10.7,9.5 11.5,10.6 11.2,11 11.5,11.2 10.9,11.8 10.5,12.5 10.7,12.7 11,12.7 11.1,12.9
10.6,14.1 9.9,15 8.2,16.7 8.8,17.2 8.4,18 8.8,18.8 8.7,19.6 8.8,20.5 8.7,20.9 8.7,21.3 9.2,21.8 8.7,22.6 8.8,23 8.8,23.2
9,23.4 8.6,23.7 8.4,24.4 8.4,24.9 7.9,25.2 7.6,26.1 7.9,26.8 8.5,27.6 6.9,28 0,28 0,0 " />
																														</svg>
																														<div
																															class="mkdf-dli-category-holder">
																															<a itemprop="url"
																																class="mkdf-dli-category"
																																href="destination-category/popular/index.html">Popular</a>
																														</div>
																														<h4 itemprop="name"
																															class="mkdf-dli-title entry-title">
																															<span>
																																<span
																																	class="mkdf-dli-title-wrap">
																																	Colorado
																																</span>
																															</span>
																														</h4>
																														<p itemprop="description"
																															class="mkdf-dli-excerpt">
																															Lorem
																															ipsum
																															dolor
																															sit
																															amet,
																															consectetur
																															adipisicing
																															elit,
																															sed
																															do
																															eiusmod
																															tempor
																														</p>
																														<div
																															class="mkdf-dli-btn-holder">
																															<a itemprop="url"
																																href="destination-item/colorado-3/index.html"
																																target="_self"
																																class="mkdf-btn mkdf-btn-medium mkdf-btn-simple mkdf-btn-svg-icon">
																																<span
																																	class="mkdf-btn-text">Read
																																	More</span>
																																<svg version="1.1"
																																	id="Layer_1"
																																	xmlns="http://www.w3.org/2000/svg"
																																	xmlns:xlink="http://www.w3.org/1999/xlink"
																																	x="0px"
																																	y="0px"
																																	viewBox="0 0 17 17"
																																	style="enable-background:new 0 0 17 17;"
																																	xml:space="preserve">
																																	<g>
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="1.7"
																																			y1="15.3"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="16"
																																			y1="1.9"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="15"
																																			y1="1.9"
																																			x2="1"
																																			y2="1.9" />
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="15"
																																			y1="16"
																																			x2="15"
																																			y2="1.9" />
																																	</g>
																																</svg>
																																<svg version="1.1"
																																	id="Layer_1"
																																	xmlns="http://www.w3.org/2000/svg"
																																	xmlns:xlink="http://www.w3.org/1999/xlink"
																																	x="0px"
																																	y="0px"
																																	viewBox="0 0 17 17"
																																	style="enable-background:new 0 0 17 17;"
																																	xml:space="preserve">
																																	<g>
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="1.7"
																																			y1="15.3"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="16"
																																			y1="1.9"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="15"
																																			y1="1.9"
																																			x2="1"
																																			y2="1.9" />
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="15"
																																			y1="16"
																																			x2="15"
																																			y2="1.9" />
																																	</g>
																																</svg>
																															</a>
																														</div>
																													</div>
																												</div>
																											</div>
																										</div>
																										<a itemprop="url"
																											class="mkdf-dli-link mkdf-block-drag-link"
																											href="destination-item/colorado-3/index.html"></a>
																									</div>
																								</article>
																								<article
																									class="mkdf-dl-item mkdf-item-space">
																									<div
																										class="mkdf-dl-item-inner">
																										<div
																											class="mkdf-dli-image d-flex justify-content-center">
																											<div class="col-12 col-md-7 col-lg-8">
																												<img loading="lazy"
																													decoding="async"
																													width="1299"
																													height="714"

																													src="{{asset('frontend\img\feedback\feedback_04.jpg')}}"
																													class="rounded attachment-full size-full wp-post-image"
																													alt="a"
																													srcset="{{asset('frontend\img\feedback\feedback_04.jpg')}} 1299w, {{asset('frontend\img\feedback\feedback_04.jpg')}} 300w, {{asset('frontend\img\feedback\feedback_04.jpg')}} 768w, {{asset('frontend\img\feedback\feedback_04.jpg')}} 1024w, {{asset('frontend\img\feedback\feedback_04.jpg')}} 600w"
																													sizes="(max-width: 1299px) 100vw, 1299px" />
																											</div>
																										</div>
																										<div
																											class="mkdf-dli-text-holder">
																											<div
																												class="mkdf-dli-text-wrapper">
																												<div
																													class="mkdf-dli-text">
																													<div
																														class="mkdf-dli-text-inner">
																														<svg xmlns="http://www.w3.org/2000/svg"
																															xmlns:xlink="http://www.w3.org/1999/xlink"
																															x="0px"
																															y="0px"
																															viewBox="0 0 14.1 28"
																															style="enable-background:new 0 0 14.1 28;"
																															xml:space="preserve"
																															class="mkdf-active-hover-left">
																															<polygon
																																class="st0"
																																points="1.9,0 0,2.3 1.3,4.8 1.6,6.6 2.2,7.7 3.7,9.5 3.2,9.8 3.7,10.9 3.7,11.8 3.4,12.8 2.6,13.7 3.2,14.8
3.7,15.8 3.3,16.5 2.8,17.1 1.5,19.2 2.7,20.5 3.5,23.1 2.7,24.4 1.8,26.4 1.8,26.8 2.5,27.3 3.4,27.5 4.2,28 14.1,28 14.1,0 " />
																														</svg>
																														<svg xmlns="http://www.w3.org/2000/svg"
																															xmlns:xlink="http://www.w3.org/1999/xlink"
																															x="0px"
																															y="0px"
																															viewBox="0 0 11.5 28"
																															style="enable-background:new 0 0 11.5 28;"
																															xml:space="preserve"
																															class="mkdf-active-hover-right">
																															<polygon
																																class="st0"
																																points="9.1,0 9.3,1.1 9.7,1.9 9.7,2.3 9.9,2.7 9.9,3.3 10.1,4 10.1,4.7 10.1,5.3 9.6,5.7 9.9,6.3 10.1,6.8
10.7,7.3 10.7,7.6 10.6,7.9 10.7,8.9 10.7,9.5 11.5,10.6 11.2,11 11.5,11.2 10.9,11.8 10.5,12.5 10.7,12.7 11,12.7 11.1,12.9
10.6,14.1 9.9,15 8.2,16.7 8.8,17.2 8.4,18 8.8,18.8 8.7,19.6 8.8,20.5 8.7,20.9 8.7,21.3 9.2,21.8 8.7,22.6 8.8,23 8.8,23.2
9,23.4 8.6,23.7 8.4,24.4 8.4,24.9 7.9,25.2 7.6,26.1 7.9,26.8 8.5,27.6 6.9,28 0,28 0,0 " />
																														</svg>
																														<div
																															class="mkdf-dli-category-holder">
																															<a itemprop="url"
																																class="mkdf-dli-category"
																																href="destination-category/popular/index.html">Popular</a>
																														</div>
																														<h4 itemprop="name"
																															class="mkdf-dli-title entry-title">
																															<span>
																																<span
																																	class="mkdf-dli-title-wrap">
																																	Norway
																																</span>
																															</span>
																														</h4>
																														<p itemprop="description"
																															class="mkdf-dli-excerpt">
																															Lorem
																															ipsum
																															dolor
																															sit
																															amet,
																															consectetur
																															adipisicing
																															elit,
																															sed
																															do
																															eiusmod
																															tempor
																														</p>
																														<div
																															class="mkdf-dli-btn-holder">
																															<a itemprop="url"
																																href="destination-item/norway/index.html"
																																target="_self"
																																class="mkdf-btn mkdf-btn-medium mkdf-btn-simple mkdf-btn-svg-icon">
																																<span
																																	class="mkdf-btn-text">Read
																																	More</span>
																																<svg version="1.1"
																																	id="Layer_1"
																																	xmlns="http://www.w3.org/2000/svg"
																																	xmlns:xlink="http://www.w3.org/1999/xlink"
																																	x="0px"
																																	y="0px"
																																	viewBox="0 0 17 17"
																																	style="enable-background:new 0 0 17 17;"
																																	xml:space="preserve">
																																	<g>
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="1.7"
																																			y1="15.3"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="16"
																																			y1="1.9"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="15"
																																			y1="1.9"
																																			x2="1"
																																			y2="1.9" />
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="15"
																																			y1="16"
																																			x2="15"
																																			y2="1.9" />
																																	</g>
																																</svg>
																																<svg version="1.1"
																																	id="Layer_1"
																																	xmlns="http://www.w3.org/2000/svg"
																																	xmlns:xlink="http://www.w3.org/1999/xlink"
																																	x="0px"
																																	y="0px"
																																	viewBox="0 0 17 17"
																																	style="enable-background:new 0 0 17 17;"
																																	xml:space="preserve">
																																	<g>
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="1.7"
																																			y1="15.3"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="16"
																																			y1="1.9"
																																			x2="15"
																																			y2="1.9" />
																																		<line
																																			x1="15"
																																			y1="1.9"
																																			x2="1"
																																			y2="1.9" />
																																		<path
																																			d="M15,1.9" />
																																		<line
																																			x1="15"
																																			y1="16"
																																			x2="15"
																																			y2="1.9" />
																																	</g>
																																</svg>
																															</a>
																														</div>
																													</div>
																												</div>
																											</div>
																										</div>
																										<a itemprop="url"
																											class="mkdf-dli-link mkdf-block-drag-link"
																											href="destination-item/norway/index.html"></a>
																									</div>
																								</article>
																							</div>

																						</div>
																					</div>
																				</div>
																			</div>
																		</div>
																	</div>
																</div>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
							@php
							$url1=asset('frontend/wp-content/uploads/2020/01/h1-img-30.jpg');
							@endphp
							<div class="vc_row wpb_row vc_row-fluid mkdf-disabled-bg-image-bellow-768"
								style="background-image: url({{$url1}});background-position: center center">
								<div class="wpb_column vc_column_container vc_col-sm-12">
									<div class="vc_column-inner">
										<div class="wpb_wrapper">
											<div
												class="mkdf-elements-holder   mkdf-one-column  mkdf-responsive-mode-768 ">
												<div class="mkdf-eh-item    " data-item-class="mkdf-eh-custom-5392"
													data-1400-1600="91px 0 128px 0" data-1025-1399="91px 0 128px 0"
													data-769-1024="91px 0 128px 0" data-681-768="51px 0 58px 0"
													data-680="0 0 30px 0">
													<div class="mkdf-eh-item-inner">
														<div class="mkdf-eh-item-content mkdf-eh-custom-5392"
															style="padding: 91px 0 128px 0">
															<div class="mkdf-horizontal-timeline-wrapper">
																<div class="mkdf-horizontal-timeline">
																	<a class="mkdf-ht-content-item"
																		href="destination-item/sweden/index.html"
																		target="_self">
																		<div
																			class="mkdf-hti-content-item-inner mkdf-timeline-has-image">
																			<div class="mkdf-hti-content-item-wrap">
																				<div class="mkdf-hti-content-image">
																					<img loading="lazy" decoding="async"
																						width="108" height="65"
																						src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-25.png')}}"
																						class="attachment-full size-full"
																						alt="s" />
																				</div>
																				<div class="mkdf-hti-content-value">
																					<h6 class="mkdf-hti-title">
																						Sweden</h6>
																					<p class="mkdf-hti-label-one">
																						47.1388422</p>
																					<p class="mkdf-hti-label-two">
																						72.9917328</p>
																				</div>
																			</div>
																			<div class="mkdf-hti-pin"></div>
																		</div>
																	</a><a class="mkdf-ht-content-item"
																		href="destination-item/colorado/index.html"
																		target="_self">
																		<div
																			class="mkdf-hti-content-item-inner mkdf-timeline-has-image">
																			<div class="mkdf-hti-content-item-wrap">
																				<div class="mkdf-hti-content-image">
																					<img loading="lazy" decoding="async"
																						width="107" height="65"
																						src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-03.png')}}"
																						class="attachment-full size-full')}}"
																						alt="a" />
																				</div>
																				<div class="mkdf-hti-content-value">
																					<h6 class="mkdf-hti-title">
																						USA</h6>
																					<p class="mkdf-hti-label-one">
																						47.1388422</p>
																					<p class="mkdf-hti-label-two">
																						72.9917328</p>
																				</div>
																			</div>
																			<div class="mkdf-hti-pin"></div>
																		</div>
																	</a><a class="mkdf-ht-content-item"
																		href="destination-item/spain/index.html"
																		target="_self">
																		<div
																			class="mkdf-hti-content-item-inner mkdf-timeline-has-image">
																			<div class="mkdf-hti-content-item-wrap">
																				<div class="mkdf-hti-content-image">
																					<img loading="lazy" decoding="async"
																						width="108" height="65"
																						src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-04.png')}}"
																						class="attachment-full size-full"
																						alt="m" />
																				</div>
																				<div class="mkdf-hti-content-value">
																					<h6 class="mkdf-hti-title">
																						Spain</h6>
																					<p class="mkdf-hti-label-one">
																						47.1388422</p>
																					<p class="mkdf-hti-label-two">
																						72.9917328</p>
																				</div>
																			</div>
																			<div class="mkdf-hti-pin"></div>
																		</div>
																	</a><a class="mkdf-ht-content-item"
																		href="destination-item/spain-2/index.html"
																		target="_self">
																		<div
																			class="mkdf-hti-content-item-inner mkdf-timeline-has-image">
																			<div class="mkdf-hti-content-item-wrap">
																				<div class="mkdf-hti-content-image">
																					<img loading="lazy" decoding="async"
																						width="107" height="65"
																						src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-05.png')}}"
																						class="attachment-full size-full"
																						alt="g" />
																				</div>
																				<div class="mkdf-hti-content-value">
																					<h6 class="mkdf-hti-title">
																						Brazil</h6>
																					<p class="mkdf-hti-label-one">
																						47.1388422</p>
																					<p class="mkdf-hti-label-two">
																						72.9917328</p>
																				</div>
																			</div>
																			<div class="mkdf-hti-pin"></div>
																		</div>
																	</a><a class="mkdf-ht-content-item"
																		href="destination-item/india-2/index.html"
																		target="_self">
																		<div
																			class="mkdf-hti-content-item-inner mkdf-timeline-has-image">
																			<div class="mkdf-hti-content-item-wrap">
																				<div class="mkdf-hti-content-image">
																					<img loading="lazy" decoding="async"
																						width="108" height="66"
																						src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-06.png')}}"
																						class="attachment-full size-full"
																						alt="s" />
																				</div>
																				<div class="mkdf-hti-content-value">
																					<h6 class="mkdf-hti-title">
																						Indonesia</h6>
																					<p class="mkdf-hti-label-one">
																						47.1388422</p>
																					<p class="mkdf-hti-label-two">
																						72.9917328</p>
																				</div>
																			</div>
																			<div class="mkdf-hti-pin"></div>
																		</div>
																	</a><a class="mkdf-ht-content-item"
																		href="destination-item/iceland/index.html"
																		target="_self">
																		<div
																			class="mkdf-hti-content-item-inner mkdf-timeline-has-image">
																			<div class="mkdf-hti-content-item-wrap">
																				<div class="mkdf-hti-content-image">
																					<img loading="lazy" decoding="async"
																						width="108" height="65"
																						src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-25-1.png')}}"
																						class="attachment-full size-full"
																						alt="w" />
																				</div>
																				<div class="mkdf-hti-content-value">
																					<h6 class="mkdf-hti-title">
																						Great Britain</h6>
																					<p class="mkdf-hti-label-one">
																						47.1388422</p>
																					<p class="mkdf-hti-label-two">
																						72.9917328</p>
																				</div>
																			</div>
																			<div class="mkdf-hti-pin"></div>
																		</div>
																	</a><a class="mkdf-ht-content-item"
																		href="destination-item/georgia/index.html"
																		target="_self">
																		<div
																			class="mkdf-hti-content-item-inner mkdf-timeline-has-image">
																			<div class="mkdf-hti-content-item-wrap">
																				<div class="mkdf-hti-content-image">
																					<img loading="lazy" decoding="async"
																						width="108" height="65"
																						src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-26.png')}}"
																						class="attachment-full size-full"
																						alt="w" />
																				</div>
																				<div class="mkdf-hti-content-value">
																					<h6 class="mkdf-hti-title">
																						France</h6>
																					<p class="mkdf-hti-label-one">
																						47.1388422</p>
																					<p class="mkdf-hti-label-two">
																						72.9917328</p>
																				</div>
																			</div>
																			<div class="mkdf-hti-pin"></div>
																		</div>
																	</a><a class="mkdf-ht-content-item"
																		href="destination-item/kazakhstan/index.html"
																		target="_self">
																		<div
																			class="mkdf-hti-content-item-inner mkdf-timeline-has-image">
																			<div class="mkdf-hti-content-item-wrap">
																				<div class="mkdf-hti-content-image">
																					<img loading="lazy" decoding="async"
																						width="108" height="65"
																						src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-27.png')}}"
																						class="attachment-full size-full"
																						alt="w" />
																				</div>
																				<div class="mkdf-hti-content-value">
																					<h6 class="mkdf-hti-title">
																						Germany</h6>
																					<p class="mkdf-hti-label-one">
																						47.1388422</p>
																					<p class="mkdf-hti-label-two">
																						72.9917328</p>
																				</div>
																			</div>
																			<div class="mkdf-hti-pin"></div>
																		</div>
																	</a><a class="mkdf-ht-content-item"
																		href="destination-item/ireland/index.html"
																		target="_self">
																		<div
																			class="mkdf-hti-content-item-inner mkdf-timeline-has-image">
																			<div class="mkdf-hti-content-item-wrap">
																				<div class="mkdf-hti-content-image">
																					<img loading="lazy" decoding="async"
																						width="108" height="65"
																						src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-28.png')}}"
																						class="attachment-full size-full"
																						alt="w" />
																				</div>
																				<div class="mkdf-hti-content-value">
																					<h6 class="mkdf-hti-title">
																						Mexico</h6>
																					<p class="mkdf-hti-label-one">
																						47.1388422</p>
																					<p class="mkdf-hti-label-two">
																						72.9917328</p>
																				</div>
																			</div>
																			<div class="mkdf-hti-pin"></div>
																		</div>
																	</a>
																</div>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
							<div class="vc_row wpb_row vc_row-fluid vc_custom_1571989261998 vc_row-has-fill">
								<div class="wpb_column vc_column_container vc_col-sm-12">
									<div class="vc_column-inner">
										<div class="wpb_wrapper">
											<div class="mkdf-row-grid-section-wrapper ">
												<div class="mkdf-row-grid-section">
													<div
														class="vc_row wpb_row vc_inner vc_row-fluid vc_custom_1579263726811">
														<div class="wpb_column vc_column_container vc_col-sm-12">
															<div class="vc_column-inner">
																<div class="wpb_wrapper">
																	<div
																		class="mkdf-destination-category-list-holder mkdf-grid-list mkdf-six-columns mkdf-normal-space">
																		<div
																			class="mkdf-dcl-inner mkdf-outer-space clearfix">
																			<article
																				class="mkdf-dcl-item mkdf-item-space">
																				<div class="mkdf-dcl-item-inner">
																					<div class="mkdf-dcli-image">
																						<img loading="lazy"
																							decoding="async" width="92"
																							height="74"
																							src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-07.png')}}"
																							class="attachment-full size-full"
																							alt="a" /> <img
																							loading="lazy"
																							decoding="async" width="92"
																							height="74"
																							src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-13.png')}}"
																							class="attachment-full size-full"
																							alt="d" /> <span
																							class="mkdf-dcl-highlight">
																							<svg xmlns="http://www.w3.org/2000/svg"
																								xmlns:xlink="http://www.w3.org/1999/xlink"
																								x="0px" y="0px"
																								viewBox="0 0 14.1 28"
																								style="enable-background:new 0 0 14.1 28;"
																								xml:space="preserve"
																								class="mkdf-active-hover-left">
																								<polygon class="st0"
																									points="1.9,0 0,2.3 1.3,4.8 1.6,6.6 2.2,7.7 3.7,9.5 3.2,9.8 3.7,10.9 3.7,11.8 3.4,12.8 2.6,13.7 3.2,14.8
3.7,15.8 3.3,16.5 2.8,17.1 1.5,19.2 2.7,20.5 3.5,23.1 2.7,24.4 1.8,26.4 1.8,26.8 2.5,27.3 3.4,27.5 4.2,28 14.1,28 14.1,0 " />
																							</svg> <span
																								class="mkdf-active-hover-middle"></span>
																							<svg xmlns="http://www.w3.org/2000/svg"
																								xmlns:xlink="http://www.w3.org/1999/xlink"
																								x="0px" y="0px"
																								viewBox="0 0 11.5 28"
																								style="enable-background:new 0 0 11.5 28;"
																								xml:space="preserve"
																								class="mkdf-active-hover-right">
																								<polygon class="st0"
																									points="9.1,0 9.3,1.1 9.7,1.9 9.7,2.3 9.9,2.7 9.9,3.3 10.1,4 10.1,4.7 10.1,5.3 9.6,5.7 9.9,6.3 10.1,6.8
10.7,7.3 10.7,7.6 10.6,7.9 10.7,8.9 10.7,9.5 11.5,10.6 11.2,11 11.5,11.2 10.9,11.8 10.5,12.5 10.7,12.7 11,12.7 11.1,12.9
10.6,14.1 9.9,15 8.2,16.7 8.8,17.2 8.4,18 8.8,18.8 8.7,19.6 8.8,20.5 8.7,20.9 8.7,21.3 9.2,21.8 8.7,22.6 8.8,23 8.8,23.2
9,23.4 8.6,23.7 8.4,24.4 8.4,24.9 7.9,25.2 7.6,26.1 7.9,26.8 8.5,27.6 6.9,28 0,28 0,0 " />
																							</svg> </span>
																					</div>
																					<div class="mkdf-dcli-text-holder">
																						<div
																							class="mkdf-dcli-text-wrapper">
																							<div class="mkdf-dcli-text">
																								<p
																									class="mkdf-dcli-count">
																									1
																									destination
																								</p>
																								<h6 itemprop="name"
																									class="mkdf-dcli-title entry-title">
																									Adventure
																								</h6>
																							</div>
																						</div>
																					</div>
																					<a itemprop="url"
																						class="mkdf-dcli-link"
																						href="destination-category/adventure/index.html"></a>
																				</div>
																			</article>
																			<article
																				class="mkdf-dcl-item mkdf-item-space">
																				<div class="mkdf-dcl-item-inner">
																					<div class="mkdf-dcli-image">
																						<img loading="lazy"
																							decoding="async" width="92"
																							height="74"
																							src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-08.png')}}"
																							class="attachment-full size-full"
																							alt="a" /> <img
																							loading="lazy"
																							decoding="async" width="92"
																							height="74"
																							src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-14.png')}}"
																							class="attachment-full size-full"
																							alt="d" /> <span
																							class="mkdf-dcl-highlight">
																							<svg xmlns="http://www.w3.org/2000/svg"
																								xmlns:xlink="http://www.w3.org/1999/xlink"
																								x="0px" y="0px"
																								viewBox="0 0 14.1 28"
																								style="enable-background:new 0 0 14.1 28;"
																								xml:space="preserve"
																								class="mkdf-active-hover-left">
																								<polygon class="st0"
																									points="1.9,0 0,2.3 1.3,4.8 1.6,6.6 2.2,7.7 3.7,9.5 3.2,9.8 3.7,10.9 3.7,11.8 3.4,12.8 2.6,13.7 3.2,14.8
3.7,15.8 3.3,16.5 2.8,17.1 1.5,19.2 2.7,20.5 3.5,23.1 2.7,24.4 1.8,26.4 1.8,26.8 2.5,27.3 3.4,27.5 4.2,28 14.1,28 14.1,0 " />
																							</svg> <span
																								class="mkdf-active-hover-middle"></span>
																							<svg xmlns="http://www.w3.org/2000/svg"
																								xmlns:xlink="http://www.w3.org/1999/xlink"
																								x="0px" y="0px"
																								viewBox="0 0 11.5 28"
																								style="enable-background:new 0 0 11.5 28;"
																								xml:space="preserve"
																								class="mkdf-active-hover-right">
																								<polygon class="st0"
																									points="9.1,0 9.3,1.1 9.7,1.9 9.7,2.3 9.9,2.7 9.9,3.3 10.1,4 10.1,4.7 10.1,5.3 9.6,5.7 9.9,6.3 10.1,6.8
10.7,7.3 10.7,7.6 10.6,7.9 10.7,8.9 10.7,9.5 11.5,10.6 11.2,11 11.5,11.2 10.9,11.8 10.5,12.5 10.7,12.7 11,12.7 11.1,12.9
10.6,14.1 9.9,15 8.2,16.7 8.8,17.2 8.4,18 8.8,18.8 8.7,19.6 8.8,20.5 8.7,20.9 8.7,21.3 9.2,21.8 8.7,22.6 8.8,23 8.8,23.2
9,23.4 8.6,23.7 8.4,24.4 8.4,24.9 7.9,25.2 7.6,26.1 7.9,26.8 8.5,27.6 6.9,28 0,28 0,0 " />
																							</svg> </span>
																					</div>
																					<div class="mkdf-dcli-text-holder">
																						<div
																							class="mkdf-dcli-text-wrapper">
																							<div class="mkdf-dcli-text">
																								<p
																									class="mkdf-dcli-count">
																									3
																									destinations
																								</p>
																								<h6 itemprop="name"
																									class="mkdf-dcli-title entry-title">
																									Beaches
																								</h6>
																							</div>
																						</div>
																					</div>
																					<a itemprop="url"
																						class="mkdf-dcli-link"
																						href="destination-category/beaches/index.html"></a>
																				</div>
																			</article>
																			<article
																				class="mkdf-dcl-item mkdf-item-space">
																				<div class="mkdf-dcl-item-inner">
																					<div class="mkdf-dcli-image">
																						<img loading="lazy"
																							decoding="async" width="92"
																							height="74"
																							src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-09.png')}}"
																							class="attachment-full size-full"
																							alt="a" /> <img
																							loading="lazy"
																							decoding="async" width="92"
																							height="74"
																							src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-15.png')}}"
																							class="attachment-full size-full"
																							alt="d" /> <span
																							class="mkdf-dcl-highlight">
																							<svg xmlns="http://www.w3.org/2000/svg"
																								xmlns:xlink="http://www.w3.org/1999/xlink"
																								x="0px" y="0px"
																								viewBox="0 0 14.1 28"
																								style="enable-background:new 0 0 14.1 28;"
																								xml:space="preserve"
																								class="mkdf-active-hover-left">
																								<polygon class="st0"
																									points="1.9,0 0,2.3 1.3,4.8 1.6,6.6 2.2,7.7 3.7,9.5 3.2,9.8 3.7,10.9 3.7,11.8 3.4,12.8 2.6,13.7 3.2,14.8
3.7,15.8 3.3,16.5 2.8,17.1 1.5,19.2 2.7,20.5 3.5,23.1 2.7,24.4 1.8,26.4 1.8,26.8 2.5,27.3 3.4,27.5 4.2,28 14.1,28 14.1,0 " />
																							</svg> <span
																								class="mkdf-active-hover-middle"></span>
																							<svg xmlns="http://www.w3.org/2000/svg"
																								xmlns:xlink="http://www.w3.org/1999/xlink"
																								x="0px" y="0px"
																								viewBox="0 0 11.5 28"
																								style="enable-background:new 0 0 11.5 28;"
																								xml:space="preserve"
																								class="mkdf-active-hover-right">
																								<polygon class="st0"
																									points="9.1,0 9.3,1.1 9.7,1.9 9.7,2.3 9.9,2.7 9.9,3.3 10.1,4 10.1,4.7 10.1,5.3 9.6,5.7 9.9,6.3 10.1,6.8
10.7,7.3 10.7,7.6 10.6,7.9 10.7,8.9 10.7,9.5 11.5,10.6 11.2,11 11.5,11.2 10.9,11.8 10.5,12.5 10.7,12.7 11,12.7 11.1,12.9
10.6,14.1 9.9,15 8.2,16.7 8.8,17.2 8.4,18 8.8,18.8 8.7,19.6 8.8,20.5 8.7,20.9 8.7,21.3 9.2,21.8 8.7,22.6 8.8,23 8.8,23.2
9,23.4 8.6,23.7 8.4,24.4 8.4,24.9 7.9,25.2 7.6,26.1 7.9,26.8 8.5,27.6 6.9,28 0,28 0,0 " />
																							</svg> </span>
																					</div>
																					<div class="mkdf-dcli-text-holder">
																						<div
																							class="mkdf-dcli-text-wrapper">
																							<div class="mkdf-dcli-text">
																								<p
																									class="mkdf-dcli-count">
																									2
																									destinations
																								</p>
																								<h6 itemprop="name"
																									class="mkdf-dcli-title entry-title">
																									Camping
																								</h6>
																							</div>
																						</div>
																					</div>
																					<a itemprop="url"
																						class="mkdf-dcli-link"
																						href="destination-category/camping-2/index.html"></a>
																				</div>
																			</article>
																			<article
																				class="mkdf-dcl-item mkdf-item-space">
																				<div class="mkdf-dcl-item-inner">
																					<div class="mkdf-dcli-image">
																						<img loading="lazy"
																							decoding="async" width="92"
																							height="74"
																							src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-10.png')}}"
																							class="attachment-full size-full"
																							alt="a" /> <img
																							loading="lazy"
																							decoding="async" width="92"
																							height="74"
																							src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-16.png')}}"
																							class="attachment-full size-full"
																							alt="d" /> <span
																							class="mkdf-dcl-highlight">
																							<svg xmlns="http://www.w3.org/2000/svg"
																								xmlns:xlink="http://www.w3.org/1999/xlink"
																								x="0px" y="0px"
																								viewBox="0 0 14.1 28"
																								style="enable-background:new 0 0 14.1 28;"
																								xml:space="preserve"
																								class="mkdf-active-hover-left">
																								<polygon class="st0"
																									points="1.9,0 0,2.3 1.3,4.8 1.6,6.6 2.2,7.7 3.7,9.5 3.2,9.8 3.7,10.9 3.7,11.8 3.4,12.8 2.6,13.7 3.2,14.8
3.7,15.8 3.3,16.5 2.8,17.1 1.5,19.2 2.7,20.5 3.5,23.1 2.7,24.4 1.8,26.4 1.8,26.8 2.5,27.3 3.4,27.5 4.2,28 14.1,28 14.1,0 " />
																							</svg> <span
																								class="mkdf-active-hover-middle"></span>
																							<svg xmlns="http://www.w3.org/2000/svg"
																								xmlns:xlink="http://www.w3.org/1999/xlink"
																								x="0px" y="0px"
																								viewBox="0 0 11.5 28"
																								style="enable-background:new 0 0 11.5 28;"
																								xml:space="preserve"
																								class="mkdf-active-hover-right">
																								<polygon class="st0"
																									points="9.1,0 9.3,1.1 9.7,1.9 9.7,2.3 9.9,2.7 9.9,3.3 10.1,4 10.1,4.7 10.1,5.3 9.6,5.7 9.9,6.3 10.1,6.8
10.7,7.3 10.7,7.6 10.6,7.9 10.7,8.9 10.7,9.5 11.5,10.6 11.2,11 11.5,11.2 10.9,11.8 10.5,12.5 10.7,12.7 11,12.7 11.1,12.9
10.6,14.1 9.9,15 8.2,16.7 8.8,17.2 8.4,18 8.8,18.8 8.7,19.6 8.8,20.5 8.7,20.9 8.7,21.3 9.2,21.8 8.7,22.6 8.8,23 8.8,23.2
9,23.4 8.6,23.7 8.4,24.4 8.4,24.9 7.9,25.2 7.6,26.1 7.9,26.8 8.5,27.6 6.9,28 0,28 0,0 " />
																							</svg> </span>
																					</div>
																					<div class="mkdf-dcli-text-holder">
																						<div
																							class="mkdf-dcli-text-wrapper">
																							<div class="mkdf-dcli-text">
																								<p
																									class="mkdf-dcli-count">
																									1
																									destination
																								</p>
																								<h6 itemprop="name"
																									class="mkdf-dcli-title entry-title">
																									Friendly
																								</h6>
																							</div>
																						</div>
																					</div>
																					<a itemprop="url"
																						class="mkdf-dcli-link"
																						href="destination-category/friendly/index.html"></a>
																				</div>
																			</article>
																			<article
																				class="mkdf-dcl-item mkdf-item-space">
																				<div class="mkdf-dcl-item-inner">
																					<div class="mkdf-dcli-image">
																						<img loading="lazy"
																							decoding="async" width="92"
																							height="74"
																							src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-11.png')}}"
																							class="attachment-full size-full"
																							alt="a" /> <img
																							loading="lazy"
																							decoding="async" width="92"
																							height="74"
																							src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-17.png')}}"
																							class="attachment-full size-full"
																							alt="d" /> <span
																							class="mkdf-dcl-highlight">
																							<svg xmlns="http://www.w3.org/2000/svg"
																								xmlns:xlink="http://www.w3.org/1999/xlink"
																								x="0px" y="0px"
																								viewBox="0 0 14.1 28"
																								style="enable-background:new 0 0 14.1 28;"
																								xml:space="preserve"
																								class="mkdf-active-hover-left">
																								<polygon class="st0"
																									points="1.9,0 0,2.3 1.3,4.8 1.6,6.6 2.2,7.7 3.7,9.5 3.2,9.8 3.7,10.9 3.7,11.8 3.4,12.8 2.6,13.7 3.2,14.8
3.7,15.8 3.3,16.5 2.8,17.1 1.5,19.2 2.7,20.5 3.5,23.1 2.7,24.4 1.8,26.4 1.8,26.8 2.5,27.3 3.4,27.5 4.2,28 14.1,28 14.1,0 " />
																							</svg> <span
																								class="mkdf-active-hover-middle"></span>
																							<svg xmlns="http://www.w3.org/2000/svg"
																								xmlns:xlink="http://www.w3.org/1999/xlink"
																								x="0px" y="0px"
																								viewBox="0 0 11.5 28"
																								style="enable-background:new 0 0 11.5 28;"
																								xml:space="preserve"
																								class="mkdf-active-hover-right">
																								<polygon class="st0"
																									points="9.1,0 9.3,1.1 9.7,1.9 9.7,2.3 9.9,2.7 9.9,3.3 10.1,4 10.1,4.7 10.1,5.3 9.6,5.7 9.9,6.3 10.1,6.8
10.7,7.3 10.7,7.6 10.6,7.9 10.7,8.9 10.7,9.5 11.5,10.6 11.2,11 11.5,11.2 10.9,11.8 10.5,12.5 10.7,12.7 11,12.7 11.1,12.9
10.6,14.1 9.9,15 8.2,16.7 8.8,17.2 8.4,18 8.8,18.8 8.7,19.6 8.8,20.5 8.7,20.9 8.7,21.3 9.2,21.8 8.7,22.6 8.8,23 8.8,23.2
9,23.4 8.6,23.7 8.4,24.4 8.4,24.9 7.9,25.2 7.6,26.1 7.9,26.8 8.5,27.6 6.9,28 0,28 0,0 " />
																							</svg> </span>
																					</div>
																					<div class="mkdf-dcli-text-holder">
																						<div
																							class="mkdf-dcli-text-wrapper">
																							<div class="mkdf-dcli-text">
																								<p
																									class="mkdf-dcli-count">
																									9
																									destinations
																								</p>
																								<h6 itemprop="name"
																									class="mkdf-dcli-title entry-title">
																									Low
																									budget
																								</h6>
																							</div>
																						</div>
																					</div>
																					<a itemprop="url"
																						class="mkdf-dcli-link"
																						href="destination-category/low-budget/index.html"></a>
																				</div>
																			</article>
																			<article
																				class="mkdf-dcl-item mkdf-item-space">
																				<div class="mkdf-dcl-item-inner">
																					<div class="mkdf-dcli-image">
																						<img loading="lazy"
																							decoding="async" width="92"
																							height="74"
																							src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-12.png')}}"
																							class="attachment-full size-full"
																							alt="a" /> <img
																							loading="lazy"
																							decoding="async" width="92"
																							height="74"
																							src="{{asset('frontend/wp-content/uploads/2019/10/h1-img-18.png')}}"
																							class="attachment-full size-full"
																							alt="d" /> <span
																							class="mkdf-dcl-highlight">
																							<svg xmlns="http://www.w3.org/2000/svg"
																								xmlns:xlink="http://www.w3.org/1999/xlink"
																								x="0px" y="0px"
																								viewBox="0 0 14.1 28"
																								style="enable-background:new 0 0 14.1 28;"
																								xml:space="preserve"
																								class="mkdf-active-hover-left">
																								<polygon class="st0"
																									points="1.9,0 0,2.3 1.3,4.8 1.6,6.6 2.2,7.7 3.7,9.5 3.2,9.8 3.7,10.9 3.7,11.8 3.4,12.8 2.6,13.7 3.2,14.8
3.7,15.8 3.3,16.5 2.8,17.1 1.5,19.2 2.7,20.5 3.5,23.1 2.7,24.4 1.8,26.4 1.8,26.8 2.5,27.3 3.4,27.5 4.2,28 14.1,28 14.1,0 " />
																							</svg> <span
																								class="mkdf-active-hover-middle"></span>
																							<svg xmlns="http://www.w3.org/2000/svg"
																								xmlns:xlink="http://www.w3.org/1999/xlink"
																								x="0px" y="0px"
																								viewBox="0 0 11.5 28"
																								style="enable-background:new 0 0 11.5 28;"
																								xml:space="preserve"
																								class="mkdf-active-hover-right">
																								<polygon class="st0"
																									points="9.1,0 9.3,1.1 9.7,1.9 9.7,2.3 9.9,2.7 9.9,3.3 10.1,4 10.1,4.7 10.1,5.3 9.6,5.7 9.9,6.3 10.1,6.8
10.7,7.3 10.7,7.6 10.6,7.9 10.7,8.9 10.7,9.5 11.5,10.6 11.2,11 11.5,11.2 10.9,11.8 10.5,12.5 10.7,12.7 11,12.7 11.1,12.9
10.6,14.1 9.9,15 8.2,16.7 8.8,17.2 8.4,18 8.8,18.8 8.7,19.6 8.8,20.5 8.7,20.9 8.7,21.3 9.2,21.8 8.7,22.6 8.8,23 8.8,23.2
9,23.4 8.6,23.7 8.4,24.4 8.4,24.9 7.9,25.2 7.6,26.1 7.9,26.8 8.5,27.6 6.9,28 0,28 0,0 " />
																							</svg> </span>
																					</div>
																					<div class="mkdf-dcli-text-holder">
																						<div
																							class="mkdf-dcli-text-wrapper">
																							<div class="mkdf-dcli-text">
																								<p
																									class="mkdf-dcli-count">
																									4
																									destinations
																								</p>
																								<h6 itemprop="name"
																									class="mkdf-dcli-title entry-title">
																									Popular
																								</h6>
																							</div>
																						</div>
																					</div>
																					<a itemprop="url"
																						class="mkdf-dcli-link"
																						href="destination-category/popular/index.html"></a>
																				</div>
																			</article>
																		</div>
																	</div>
																</div>
															</div>
														</div>
													</div>
												</div>
											</div>
											<div class="mkdf-row-grid-section-wrapper ">
												<div class="mkdf-row-grid-section">
													<div class="vc_row wpb_row vc_inner vc_row-fluid">
														<div
															class="wpb_column vc_column_container vc_col-sm-12 vc_col-lg-offset-0 vc_col-lg-8 vc_col-md-offset-0 vc_col-md-12 vc_col-sm-offset-0">
															<div class="vc_column-inner">
																<div class="wpb_wrapper">
																	<div class="mkdf-blog-list-holder mkdf-grid-list mkdf-bl-standard mkdf-two-columns mkdf-disable-bottom-space mkdf-small-space mkdf-bl-pag-no-pagination "
																		data-type=standard data-number-of-posts=4
																		data-number-of-columns=two
																		data-space-between-items=small
																		data-category=destinations data-orderby=date
																		data-order=ASC data-image-size=full
																		data-title-tag=h5 data-excerpt-length=0
																		data-post-info-section=yes
																		data-post-info-image=yes
																		data-post-info-author=yes
																		data-post-info-date=yes
																		data-post-info-category=no
																		data-post-info-comments=no
																		data-post-info-like=no data-post-info-share=no
																		data-pagination-type=no-pagination
																		data-max-num-pages=1 data-next-page=2>
																		<div class="mkdf-bl-wrapper mkdf-outer-space">
																			<ul class="mkdf-blog-list">
																				<li
																					class="mkdf-bl-item mkdf-item-space">
																					<div class="mkdf-bli-inner">
																						<div class="mkdf-bli-wrapper">

																							<div
																								class="mkdf-post-image">
																								<a itemprop="url"
																									href="essential-skiing-tips-for-beginners-novices/index.html"
																									title="Essential Skiing Tips For Beginners &#038; Novices">
																									<img loading="lazy"
																										decoding="async"
																										width="1300"
																										height="851"
																										src="{{asset('frontend/wp-content/uploads/2019/10/blog-post-img-06-1.jpg')}}"
																										class="attachment-full size-full wp-post-image"
																										alt="s"
																										srcset="https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-06-1.jpg 1300w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-06-1-600x393.jpg 600w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-06-1-300x196.jpg 300w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-06-1-768x503.jpg 768w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-06-1-1024x670.jpg 1024w"
																										sizes="(max-width: 1300px) 100vw, 1300px" />
																								</a>
																							</div>
																						</div>
																						<div class="mkdf-bli-content">
																							<div class="mkdf-bli-info">
																								<div itemprop="dateCreated"
																									class="mkdf-post-info-date entry-date published updated">
																									<span
																										aria-hidden="true"
																										class="mkdf-icon-font-elegant icon_calendar "></span>
																									<a itemprop="url"
																										href="2019/10/index.html">
																										October
																										10,
																										2019
																									</a>
																									<meta
																										itemprop="interactionCount"
																										content="UserComments: 0" />
																								</div>
																								<div
																									class="mkdf-post-info-author">
																									<span
																										aria-hidden="true"
																										class="mkdf-icon-font-elegant icon_pencil "></span>
																									<a itemprop="author"
																										class="mkdf-post-info-author-link"
																										href="author/alisa-michaels/index.html">
																										<span
																											class="mkdf-post-info-author-text">
																											by
																										</span>
																										Alisa
																										Michaels
																									</a>
																								</div>
																							</div>

																							<h5 itemprop="name"
																								class="entry-title mkdf-post-title">
																								<a itemprop="url"
																									href="essential-skiing-tips-for-beginners-novices/index.html"
																									title="Essential Skiing Tips For Beginners &#038; Novices">
																									Essential
																									Skiing
																									Tips For
																									Beginners
																									&#038;
																									Novices
																								</a>
																							</h5>
																							<div
																								class="mkdf-bli-excerpt">
																							</div>
																						</div>
																					</div>
																				</li>
																				<li
																					class="mkdf-bl-item mkdf-item-space">
																					<div class="mkdf-bli-inner">
																						<div class="mkdf-bli-wrapper">

																							<div
																								class="mkdf-post-image">
																								<a itemprop="url"
																									href="hidden-hiking-routs-in-the-outscirts-of-your-city-2/index.html"
																									title="Hidden hiking routs in the  outscirts of your city">
																									<img loading="lazy"
																										decoding="async"
																										width="1300"
																										height="851"
																										src="{{asset('frontend/wp-content/uploads/2019/10/blog-post-img-07-1.jpg')}}"
																										class="attachment-full size-full wp-post-image"
																										alt="s"
																										srcset="https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-07-1.jpg 1300w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-07-1-600x393.jpg 600w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-07-1-300x196.jpg 300w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-07-1-768x503.jpg 768w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-07-1-1024x670.jpg 1024w"
																										sizes="(max-width: 1300px) 100vw, 1300px" />
																								</a>
																							</div>
																						</div>
																						<div class="mkdf-bli-content">
																							<div class="mkdf-bli-info">
																								<div itemprop="dateCreated"
																									class="mkdf-post-info-date entry-date published updated">
																									<span
																										aria-hidden="true"
																										class="mkdf-icon-font-elegant icon_calendar "></span>
																									<a itemprop="url"
																										href="2019/10/index.html">
																										October
																										10,
																										2019
																									</a>
																									<meta
																										itemprop="interactionCount"
																										content="UserComments: 0" />
																								</div>
																								<div
																									class="mkdf-post-info-author">
																									<span
																										aria-hidden="true"
																										class="mkdf-icon-font-elegant icon_pencil "></span>
																									<a itemprop="author"
																										class="mkdf-post-info-author-link"
																										href="author/alisa-michaels/index.html">
																										<span
																											class="mkdf-post-info-author-text">
																											by
																										</span>
																										Alisa
																										Michaels
																									</a>
																								</div>
																							</div>

																							<h5 itemprop="name"
																								class="entry-title mkdf-post-title">
																								<a itemprop="url"
																									href="hidden-hiking-routs-in-the-outscirts-of-your-city-2/index.html"
																									title="Hidden hiking routs in the  outscirts of your city">
																									Hidden
																									hiking
																									routs in
																									the
																									outscirts
																									of your
																									city
																								</a>
																							</h5>
																							<div
																								class="mkdf-bli-excerpt">
																							</div>
																						</div>
																					</div>
																				</li>
																				<li
																					class="mkdf-bl-item mkdf-item-space">
																					<div class="mkdf-bli-inner">
																						<div class="mkdf-bli-wrapper">

																							<div
																								class="mkdf-post-image">
																								<a itemprop="url"
																									href="rock-climbing-is-one-of-the-best-hobbies-for-creatives/index.html"
																									title="Rock climbing is one of the  best hobbies for creatives">
																									<img loading="lazy"
																										decoding="async"
																										width="1300"
																										height="851"
																										src="{{asset('frontend/wp-content/uploads/2019/10/blog-post-img-08-1.jpg')}}"
																										class="attachment-full size-full wp-post-image"
																										alt="s"
																										srcset="https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-08-1.jpg 1300w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-08-1-600x393.jpg 600w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-08-1-300x196.jpg 300w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-08-1-768x503.jpg 768w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-08-1-1024x670.jpg 1024w"
																										sizes="(max-width: 1300px) 100vw, 1300px" />
																								</a>
																							</div>
																						</div>
																						<div class="mkdf-bli-content">
																							<div class="mkdf-bli-info">
																								<div itemprop="dateCreated"
																									class="mkdf-post-info-date entry-date published updated">
																									<span
																										aria-hidden="true"
																										class="mkdf-icon-font-elegant icon_calendar "></span>
																									<a itemprop="url"
																										href="2019/10/index.html">
																										October
																										10,
																										2019
																									</a>
																									<meta
																										itemprop="interactionCount"
																										content="UserComments: 0" />
																								</div>
																								<div
																									class="mkdf-post-info-author">
																									<span
																										aria-hidden="true"
																										class="mkdf-icon-font-elegant icon_pencil "></span>
																									<a itemprop="author"
																										class="mkdf-post-info-author-link"
																										href="author/alisa-michaels/index.html">
																										<span
																											class="mkdf-post-info-author-text">
																											by
																										</span>
																										Alisa
																										Michaels
																									</a>
																								</div>
																							</div>

																							<h5 itemprop="name"
																								class="entry-title mkdf-post-title">
																								<a itemprop="url"
																									href="rock-climbing-is-one-of-the-best-hobbies-for-creatives/index.html"
																									title="Rock climbing is one of the  best hobbies for creatives">
																									Rock
																									climbing
																									is one
																									of the
																									best
																									hobbies
																									for
																									creatives
																								</a>
																							</h5>
																							<div
																								class="mkdf-bli-excerpt">
																							</div>
																						</div>
																					</div>
																				</li>
																				<li
																					class="mkdf-bl-item mkdf-item-space">
																					<div class="mkdf-bli-inner">
																						<div class="mkdf-bli-wrapper">

																							<div
																								class="mkdf-post-image">
																								<a itemprop="url"
																									href="hidden-gems-you-must-visit-while-explorig-greece/index.html"
																									title="Hidden gems you must visit while explorig greece">
																									<img loading="lazy"
																										decoding="async"
																										width="1300"
																										height="851"
																										src="{{asset('frontend/wp-content/uploads/2019/10/blog-post-img-09-1.jpg')}}"
																										class="attachment-full size-full wp-post-image"
																										alt="s"
																										srcset="https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-09-1.jpg 1300w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-09-1-600x393.jpg 600w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-09-1-300x196.jpg 300w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-09-1-768x503.jpg 768w, https://wanderland.qodeinteractive.com/wp-content/uploads/2019/10/blog-post-img-09-1-1024x670.jpg 1024w"
																										sizes="(max-width: 1300px) 100vw, 1300px" />
																								</a>
																							</div>
																						</div>
																						<div class="mkdf-bli-content">
																							<div class="mkdf-bli-info">
																								<div itemprop="dateCreated"
																									class="mkdf-post-info-date entry-date published updated">
																									<span
																										aria-hidden="true"
																										class="mkdf-icon-font-elegant icon_calendar "></span>
																									<a itemprop="url"
																										href="2019/10/index.html">
																										October
																										10,
																										2019
																									</a>
																									<meta
																										itemprop="interactionCount"
																										content="UserComments: 0" />
																								</div>
																								<div
																									class="mkdf-post-info-author">
																									<span
																										aria-hidden="true"
																										class="mkdf-icon-font-elegant icon_pencil "></span>
																									<a itemprop="author"
																										class="mkdf-post-info-author-link"
																										href="author/alisa-michaels/index.html">
																										<span
																											class="mkdf-post-info-author-text">
																											by
																										</span>
																										Alisa
																										Michaels
																									</a>
																								</div>
																							</div>

																							<h5 itemprop="name"
																								class="entry-title mkdf-post-title">
																								<a itemprop="url"
																									href="hidden-gems-you-must-visit-while-explorig-greece/index.html"
																									title="Hidden gems you must visit while explorig greece">
																									Hidden
																									gems you
																									must
																									visit
																									while
																									explorig
																									greece
																								</a>
																							</h5>
																							<div
																								class="mkdf-bli-excerpt">
																							</div>
																						</div>
																					</div>
																				</li>
																			</ul>
																		</div>
																	</div>
																	<div class="vc_empty_space" style="height: 45px">
																		<span class="vc_empty_space_inner"></span>
																	</div>
																</div>
															</div>
														</div>
														<div
															class="wpb_column vc_column_container vc_col-sm-12 vc_col-lg-offset-0 vc_col-lg-4 vc_col-md-offset-0 vc_col-md-12 vc_col-sm-offset-0">
															<div class="vc_column-inner vc_custom_1571818429894">
																<div class="wpb_wrapper">
																	<div class="mkdf-section-title-holder  "
																		style="text-align: left">
																		<div class="mkdf-st-inner">
																			<span class="mkdf-st-tagline">Lorem
																				ipsum dolor sit amet</span>
																			<h3 class="mkdf-st-title">
																				HOW TO FIND YOUR DIGITAL
																				RESORT </h3>
																			<p class="mkdf-st-text "
																				style="font-size: 18px;line-height: 26px">
																				Lorem ipsum dolor sit amet,
																				conse ctetur nus adipisic
																				ing elit, sed do eiusmod
																				tempor incididu nt ut labore
																				et dolore magna aliqua. Ut
																				enim ad minim veniam, quis
																				nostrud exerc itation mas
																				ullamco laboris nisi.ut
																				aliquip ex ea commodo
																				consequat. Duis. </p>
																		</div>
																	</div>
																	<div class="vc_empty_space" style="height: 15px">
																		<span class="vc_empty_space_inner"></span>
																	</div>
																	<div class="mkdf-social-share-holder mkdf-list">
																		<ul>
																			<li class="mkdf-facebook-share">
																				<a itemprop="url"
																					class="mkdf-share-link" href="#"
																					onclick="">
																					<span
																						class="mkdf-social-network-icon ion-social-facebook"></span>
																				</a>
																			</li>
																			<li class="mkdf-twitter-share">
																				<a itemprop="url"
																					class="mkdf-share-link" href="#"
																					onclick="">
																					<span
																						class="mkdf-social-network-icon ion-social-twitter"></span>
																				</a>
																			</li>
																			<li class="mkdf-tumblr-share">
																				<a itemprop="url"
																					class="mkdf-share-link" href="#"
																					onclick="">
																					<span
																						class="mkdf-social-network-icon ion-social-tumblr"></span>
																				</a>
																			</li>
																			<li class="mkdf-pinterest-share">
																				<a itemprop="url"
																					class="mkdf-share-link" href="#"
																					onclick="">
																					<span
																						class="mkdf-social-network-icon ion-social-pinterest"></span>
																				</a>
																			</li>
																		</ul>
																		<p class="mkdf-social-title">Share
																		</p>
																	</div>
																	<div class="vc_empty_space" style="height: 45px">
																		<span class="vc_empty_space_inner"></span>
																	</div>
																</div>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>

		</div>

	</div> <!-- close div.content_inner -->
</div> <!-- close div.content -->